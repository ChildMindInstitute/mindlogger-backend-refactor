...
class TestStorageClients:
    ...
    async def test_select_answer_storage_client(
        self, user_factory: type[UserFactory], tom: User, session: AsyncSession, normal_storage_settings: Settings
    ) -> None:
        """Test a non-arbitrary server client"""
        timmy = user_factory.build()
        client = await select_answer_storage(owner_id=timmy.id, session=session, app_settings=normal_storage_settings)
        assert client.config.bucket == normal_storage_settings.cdn.bucket_answer
