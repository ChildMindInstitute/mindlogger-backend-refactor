...
class TestMFAEndpoints:
    ...
    async def test_mfa_totp_initiate_stores_pending_secret(self, client: TestClient, user: User, session: AsyncSession):
    ...
    async def test_mfa_totp_verify_expired_setup(self, client: TestClient, user: User, session: AsyncSession):
        ...
        assert user_db.pending_mfa_secret is not None
        await crud.update_pending_mfa(
            user_id=user.id, encrypted_secret=user_db.pending_mfa_secret, created_at=expired_time
        )
        ...
        response = await client.post(self.totp_verify_url, data={"code": valid_code})
