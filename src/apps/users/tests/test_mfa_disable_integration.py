...
class TestMFADisableIntegration:
    ...
    async def test_disable_flow_with_recovery_codes_clears_all_codes(
        self, client: TestClient, user_with_mfa: User, session: AsyncSession
    ):
        """Disabling MFA when recovery codes exist clears all recovery codes."""
        ...
        # Create recovery codes for the user
        crud = UsersCRUD(session)
        recovery_crud = RecoveryCodeCRUD(session)

        # Set recovery_codes_generated_at
        await crud.update_by_id(
            user_with_mfa.id,
            {"recovery_codes_generated_at": datetime(2024, 1, 1, 0, 0, 0)},
        )

        # Create some recovery code records using RecoveryCodeCreate
        ...
        for i in range(3):
            ...
            recovery_code_creates.append(
                RecoveryCodeCreate(
                    ...
                    code_hash=hashed,
                    code_encrypted=hashed,  # Using hash as encrypted for test simplicity
                    used=False,
                )
            )
