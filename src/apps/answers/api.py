...
def _log_answer_creation_context(user: User, schema: AppletAnswerCreate, device_id: str | None) -> None:
    """Log context for answer creation to help with debugging."""
    logger.info(
        "Answer creation initiated",
        extra={
            "event_type": "answer_creation_started",
            "user_id": str(user.id),
            "applet_id": str(schema.applet_id),
            "activity_id": str(schema.activity_id),
            "flow_id": str(schema.flow_id) if schema.flow_id else None,
            "version": schema.version,
            "submit_id": str(schema.submit_id),
            "has_device_id": device_id is not None,
            "has_target_subject_id": schema.target_subject_id is not None,
            "has_source_subject_id": schema.source_subject_id is not None,
            "has_input_subject_id": schema.input_subject_id is not None,
            "is_flow_completed": schema.is_flow_completed,
            "client_app_id": schema.client.app_id if schema.client else None,
            "client_app_version": schema.client.app_version if schema.client else None,
            "answer_item_count": len(schema.answer.item_ids) if schema.answer.item_ids else 0,
            "has_alerts": len(schema.alerts) > 0,
            "allowed_ehr_ingest": schema.allowed_ehr_ingest,
        },
    )


def _log_answer_creation_error(user: User, schema: AppletAnswerCreate, error: Exception, device_id: str | None) -> None:
    """Log detailed error information for answer creation failures."""
    logger.error(
        "Answer creation failed",
        extra={
            "event_type": "answer_creation_failed",
