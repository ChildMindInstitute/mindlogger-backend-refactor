...
from apps.girderformindlogger.constants import GIRDER_ROUTE_ID
...
class SettingDefault(object):
    ...
    defaults = {
        ...
        SettingKey.BRAND_NAME: "Girder for MindLogger",
        SettingKey.COLLECTION_CREATE_POLICY: {
            "open": False,
            "groups": [],
            "users": [],
        },
        SettingKey.CONTACT_EMAIL_ADDRESS: "matterlab@childmind.org",
        ...
        SettingKey.CORS_ALLOW_HEADERS: "Accept-Encoding, Authorization, Content-Disposition, "
        ...
        SettingKey.EMAIL_FROM_ADDRESS: "MindLogger <mindlogger@mindlogger.org>",
    ...
    def _defaultEmailHost():
        if (
            cherrypy.request
            and cherrypy.request.local
            and cherrypy.request.local.name
        ):
            host = "%s://%s" % (
...
class SettingValidator(object):
    ...
    def _validateApiKeys(doc):
        if not isinstance(doc["value"], bool):
            raise ValidationException(
                "API key setting must be boolean.", "value"
            )
    ...
    def _validateBannerColor(doc):
        if not doc["value"]:
            raise ValidationException(
                "The banner color may not be empty", "value"
            )
    ...
    def _validateBrandName(doc):
        if not doc["value"]:
            raise ValidationException(
                "The brand name may not be empty", "value"
            )
    ...
    def _validateCorsAllowHeaders(doc):
        if isinstance(doc["value"], six.string_types):
            ...
            return
        raise ValidationException(
            "Allowed headers must be a comma-separated list or an empty string.",
            "value",
        )
    ...
    def _validateCorsExposeHeaders(doc):
        if not isinstance(doc["value"], six.string_types):
            raise ValidationException(
                "CORS exposed headers must be a string", "value"
            )
    ...
    def _validateRouteTable(doc):
        ...
        if (
            GIRDER_ROUTE_ID not in doc["value"]
            or not doc["value"][GIRDER_ROUTE_ID]
        ):
    ...
    def _validateSmtpPassword(doc):
        if not isinstance(doc["value"], six.string_types):
            raise ValidationException(
                "SMTP password must be a string", "value"
            )
    ...
    def _validateSmtpUsername(doc):
        if not isinstance(doc["value"], six.string_types):
            raise ValidationException(
                "SMTP username must be a string", "value"
            )
    ...
    def _validateUserDefaultFolders(doc):
        if doc["value"] not in ("public_private", "none"):
            raise ValidationException(
                'User default folders must be either "public_private" or "none".',
                "value",
            )
