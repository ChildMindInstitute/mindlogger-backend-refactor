...
class Collection(AccessControlledModel):
    ...
    def remove(self, collection, progress=None, **kwargs):
        ...
        folders = folderModel.find(
            {"parentId": collection["_id"], "parentCollection": "collection"}
        )
        ...
        if progress:
            progress.update(
                increment=1, message="Deleted collection " + collection["name"]
            )
    ...
    def createCollection(
        ...
        """
        if reuseExisting:
            existing = self.findOne({"name": name})
            if existing:
                return existing
    ...
    def filter(self, doc, user=None, additionalKeys=None):
        ...
        """
        filteredDoc = super(Collection, self).filter(
    ...
    def subtreeCount(self, doc, includeItems=True, user=None, level=None):
        ...
        folders = folderModel.findWithPermissions(
            {"parentId": doc["_id"], "parentCollection": "collection"},
            fields="access",
            user=user,
            level=level,
        )
