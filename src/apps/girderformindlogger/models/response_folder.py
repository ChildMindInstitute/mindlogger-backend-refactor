...
from apps.girderformindlogger.models.aes_encrypt import AESEncryption
...
from apps.girderformindlogger.models.roles import getUserCipher
...
class ResponseItem(AESEncryption, Item):
    def initialize(self):
        ...
        self.ensureTextIndex({"name": 1, "description": 1})
    ...
    def decodeDocument(self, document):
        ...
        if metadata:
            if "responses" in metadata and isinstance(
                metadata["responses"], str
            ):
                metadata["responses"] = json_util.loads(metadata["responses"])

                for i in range(0, len(metadata.get("items", []))):
                    metadata["responses"][metadata["items"][i]] = metadata[
                        "responses"
                    ].pop(str(i))

            last7Days = metadata.get("last7Days", {}).get("responses")
            if last7Days and isinstance(last7Days, str):
                last7Days = metadata["last7Days"][
                    "responses"
                ] = json_util.loads(last7Days)

                for i in range(0, len(metadata.get("items", []))):
                    last7Days[str(i)][0]["date"] = last7Days[str(i)][0][
                        "date"
                    ].replace(tzinfo=None)
    ...
    def updateAESKey(self, document, baseKey):
        responseStartTime = document.get("meta", {}).get(
            "responseStarted", None
        )
        if responseStartTime:
            timestamp = (
                datetime.datetime.fromtimestamp(responseStartTime // 1000)
                .isoformat()[-32:]
                .encode("utf-8")
            )
            length = len(timestamp)
            self.AES_KEY = bytes(
                (baseKey[i] ^ timestamp[i % length] ^ 0x34)
                for i in range(0, 32)
            )
    ...
    def createResponseItem(
        ...
        if "baseParentType" not in folder:
            pathFromRoot = self.parentsToRoot(
                {"folderId": folder["_id"]}, creator, force=True
            )
...
class ResponseFolder(Folder):
    ...
    def load(
        ...
        responseFolder = Folder().setMetadata(
            responseFolder, {"responseFolder": True}
        )
        ...
        if str(user.get("_id")) not in [
            ...
            accessList.get("users", {}).append(
                {"id": str(user.get("_id")), "level": AccessType.ADMIN}
            )
        Folder().setAccessList(responseFolder, accessList)
        if applet:
            responseFolders = []
            allResponseFolders = list(
                Folder().childFolders(
