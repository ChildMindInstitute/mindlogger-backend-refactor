    ...
    if cUser is None:
        ...
        try:
            cUser = UserModel().createUser(
                login="-".join(
                    [appletName.replace(" ", ""), str(newCipher["name"])]
                ),
...
def decipherUser(appletSpecificId):
    ...
    try:
        ciphered = FolderModel().load(
            appletSpecificId, level=AccessType.NONE, user=thisUser
        )
        userId = list(
            FolderModel().find(
                query={
    ...
    try:
        cUser = (
            str(userId[0]["meta"]["user"]["@id"])
            if len(userId)
            and type(userId[0]) == dict
            and userId[0].get("meta").get("user").get("@id")
            else None
        )
        FolderModel().setUserAccess(
            doc=ciphered,
...
def getUserCipher(appletAssignment, user):
    ...
    aUser = (
        [
            ...
            if cipher.get("meta")
            and cipher["meta"].get("user")
            and cipher["meta"]["user"].get("@id")
            and cipher.get("parentId")
        ]
        if cUser and len(allCiphers)
        else []
    )
    aUser = (
        aUser[0]
...
def nextCipher(currentCiphers):
    ...
    for c in [
        cipher.get("name")
        for cipher in currentCiphers
        if cipher.get("name") is not None
    ]:
        try:
...
def userByEmail(email):
    try:
        user = UserModel().findOne(
            {"email": UserModel().hash(email), "email_encrypted": True}
        )
        if not user:
            user = UserModel().findOne(
                {
                    "email": UserModel().hash(email),
                    "email_encrypted": {"$ne": True},
                }
            )
    except:
        return None
