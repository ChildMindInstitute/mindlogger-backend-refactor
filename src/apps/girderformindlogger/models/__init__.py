...
def getDbConnection(
    uri=None, replicaSet=None, autoRetry=True, quiet=False, **kwargs
):
    """
...
def pluralize(modelType):
    return "{}s".format(
        ...
        else "{}ie".format(modelType[:-1])
        if modelType.endswith("y")
        else modelType
    )


def cycleModels(IRIset, modelType=None, meta={}):
    from apps.girderformindlogger.constants import REPROLIB_TYPES
    ...
    query = {
        ...
        "meta.{}.url".format(modelType): {"$in": list(IRIset)},
    }
    ...
    cachedDoc = (
        ...
        if modelType == "screen"
        else FolderModel().findOne(query)
    )
    ...
    modelType = [
        rt
        for rt in list(REPROLIB_TYPES.keys())
...
def smartImport(IRI, user=None, refreshCache=False, modelType=None, meta={}):
    ...
    model, modelType = MODELS[mt1]().getFromUrl(
        IRI, user=user, refreshCache=refreshCache, thread=False, meta=meta
    )
    return (
        modelType,
