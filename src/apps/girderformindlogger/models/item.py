...
class Item(acl_mixin.AccessControlMixin, Model):
    ...
    def initialize(self):
        ...
        self.ensureIndices(
            (
                ...
                ([("meta.baseAppletId", 1), ("meta.baseItemId", 1)], {}),
                (
                    [
                        ("meta.screen.@type", 1),
                        ("meta.screen.url", 1),
    ...
    def remove(self, item, **kwargs):
        ...
        uploads = uploadModel.find(
            {"parentId": item["_id"], "parentType": "item"}
        )
    ...
    def filter(self, doc, user=None, additionalKeys=None):
        ...
        """
        filteredDoc = super(Item, self).filter(
    ...
    def parentsToRoot(self, item, user=None, force=False):
        ...
        if force:
            ...
            folderIdsToRoot.append(
                {"type": "folder", "object": filteredFolder}
            )

        return folderIdsToRoot

    def copyItem(
        self, srcItem, creator, name=None, folder=None, description=None
    ):
        """
