...
class ProtoUser(User):
    ...
    def initialize(self):
        ...
        self.exposeFields(
            level=AccessType.READ, fields=("_id", "public", "email", "created")
        )
        self.exposeFields(
            level=AccessType.ADMIN, fields=("groupInvites", "status")
        )
    ...
    def validate(self, doc):
        ...
        if existing is not None:
            raise ValidationException(
                "".join(
                    ["That email is already registered:", str(existing["_id"])]
                ),
                "email",
            )
    ...
    def _sendCreateAccountEmail(self, user, email):
        ...
        text = mail_utils.renderTemplate(
            "emailCreateAccount.mako", {"url": url}
        )
        mail_utils.sendMail(
            to=email, subject="MindLogger: Invitation", text=text
        )
    ...
    def remove(self, protoUser, progress=None, **kwargs):
        ...
        if progress:
            progress.update(
                increment=1, message="Deleted protoUser " + protoUser["_id"]
            )
