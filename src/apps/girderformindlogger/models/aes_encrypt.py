...
class AESEncryption(AccessControlledModel):
    ...
    def initAES(self, fields=[], maxCount=4):
        self.baseKey = (
            cherrypy.config["aes_key"]
            if "aes_key" in cherrypy.config
            else b"a!z%C*f4JanU5kap2te45v9y/A?D(G+K"
        )
    ...
    def save(self, document, validate=True, triggerEvents=True):
        ...
        return self.decryptFields(
            super().save(document, False, triggerEvents), self.fields
        )
