...
class Upload(Model):
    ...
    def uploadFromFile(
        ...
        while True:
            ...
            upload = self.handleChunk(
                upload, RequestBodyStream(six.BytesIO(data), len(data))
            )
    ...
    def finalizeUpload(self, upload, assetstore=None):
        ...
        if "fileId" in upload:  # Updating an existing file's contents
            ...
            assetstore_utilities.getAssetstoreAdapter(
                Assetstore().load(file["assetstoreId"])
            ).deleteFile(file)
    ...
    def createUploadToFile(
        self, file, user, size, reference=None, assetstore=None
    ):
        """
    ...
    def moveFileToAssetstore(
        self, file, user, assetstore, progress=noProgress
    ):
        """
        ...
        event = events.trigger(
            "model.upload.movefile", {"file": file, "assetstore": assetstore}
        )
