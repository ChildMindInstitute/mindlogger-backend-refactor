...
from apps.girderformindlogger.exceptions import AccessException, GirderException, ValidationException
...
class Invitation(AESEncryption):
    ...
    def remove(self, invitation, progress=None, **kwargs):
        ...
        AccessControlledModel.remove(
            self, invitation, progress=progress, **kwargs
        )
        if progress:
    ...
    def createInvitation(
        self, applet, coordinator, role="user", profile=None, idCode=None
    ):
        """
        ...
        existing = (
            self.findOne({"appletId": applet["_id"], "idCode": idCode})
            if codified
            else None
        )
        ...
        return {
            k: v
            for k, v in self.save(invitation, validate=False).items()
            if (k != "idCode" and v is not None)
        }
    ...
    def createInvitationForSpecifiedUser(
        ...
        if not invitation:
            invitation = {"appletId": applet["_id"], "created": now}
            if user:
    ...
    def acceptInvitation(
        self, invitation, user, userEmail=""
    ):  # we need to save coordinator/manager's email as plain text
        from apps.girderformindlogger.models.applet import Applet
        ...
        if profile == None or not len(profile):
            profile = Profile().createProfile(
                applet, user, role=invitation.get("role", "user")
            )
                ...
                for k in invitation["schema:knows"]:
                    if k in profile["schema:knows"]:
                        profile["schema:knows"][k].extend(
                            [
                                r
                                for r in invitation["schema:knows"][k]
        ...
        if invited_role == "reviewer":
            Profile().updateReviewerList(
                profile, invitation.get("accessibleUsers", []), isMRNList=True
            )
    ...
    def accessToDuplicatedApplets(self, invitation, user, userEmail=""):
        ...
        duplicates = list(
            Applet().find({"duplicateOf": ObjectId(invitation["appletId"])})
        )
        ...
        for duplicate in duplicates:
            newInvitation = self.createInvitationForSpecifiedUser(
                applet=duplicate,
                coordinator=UserModel().load(
                    invitation["inviterId"], force=True
                ),
            ...
            self.acceptInvitation(
                self.load(newInvitation["_id"], force=True), user, userEmail
            )
    ...
    def htmlInvitation(
        ...
        """
        from apps.girderformindlogger.api.rest import getApiUrl
        ...
        from apps.girderformindlogger.utility import context as contextUtil
        from apps.girderformindlogger.utility import mail_utils

        invitation = self.findOne({"_id": ObjectId(invitationId)})

        if not invitation:
            return self.getMessageForAlreadyAcceptedInvitation(
                invitationId, invitee.get("lang", "en")
            )
        ...
        if existingProfile and (
            role == "user" or len(existingProfile.get("roles", [])) > 1
        ):
            return {
                "body": t(
                    "invitation_already_accepted",
                    invitation.get("lang", "en"),
        ...
        description = (
            ...
            .get(
                ...
                .get("meta", {})
                .get("protocol")
                if "protocol" in applet.get("meta", {})
                else {},
            )
        ...
        body = mail_utils.renderTemplate(
            f'welcome{"Owner" if role == "owner" else ""}.{invitation.get("lang", "en")}.mako',
            {
                ...
                if instanceName is not None and len(instanceName)
                else "",
                "managers": "{}".format(
        ...
        body2 = None
        try:
            body2 = mail_utils.renderTemplate(
                f'welcomeFooter.{invitation.get("lang", "en")}.mako',
                {
                    "userName": userName,
                    "isMobile": isMobile,
                    "accept": accept,
        ...
        return {
            ...
            """.format(
                    appletName=appletName, instanceName=instanceName, body=body
                ).strip()
            ),
    ...
    def getMessageForAlreadyAcceptedInvitation(self, invitationId, lang):
        from apps.girderformindlogger.models.applet import Applet
        from apps.girderformindlogger.models.profile import Profile
        ...
        if existingProfile:
            applet = Applet().load(
                ObjectId(existingProfile["appletId"]), force=True
            )
            appletName = applet["meta"]["applet"].get(
                "displayName", applet.get("displayName", "new applet")
            )
