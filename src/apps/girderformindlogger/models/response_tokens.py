...
class ResponseTokens(AESEncryption, dict):
    ...
    def getResponseTokens(
        self, profile, startDate=None, retrieveUserKeys=True
    ):
            ...
            for token in tokens:
                if "created" in token:
                    token["created"] = (
                        ...
                        .astimezone(
                            timezone(
                                timedelta(hours=profile.get("timezone", 0))
                            )
                        )
                        .isoformat()
                    )
        ...
        tokens = list(
            self.find(
                query,
                fields=["created", "data", "date", "userPublicKey"]
                if retrieveUserKeys
                else ["data", "date"],
        ...
        trackers = list(
            self.find(
                query,
                fields=["created", "data", "userPublicKey"]
                if retrieveUserKeys
                else ["created", "data"],
            )
        )
        ...
        trackerAggregation = list(
            self.find(
                query,
                fields=["created", "data", "userPublicKey", "date"]
                if retrieveUserKeys
                else ["created", "data", "date"],
            )
        )
