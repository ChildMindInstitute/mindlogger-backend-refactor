...
from apps.girderformindlogger.utility._cache import rateLimitBuffer
...
class User(AESEncryption):
    ...
    def initialize(self):
        ...
        self.exposeFields(
            ...
            fields=("size", "status", "emailVerified", "creatorId"),
        )
        ...
        self.initAES(
            [("firstName", 64), ("lastName", 64), ("displayName", 64)]
        )
    ...
    def validate(self, doc):
        ...
        if not doc["displayName"]:
            raise ValidationException(
                "Display name must not be empty.", "displayName"
            )
        ...
        if (
            not doc["email_encrypted"]
            and len(doc["email"])
            and not mail_utils.validateEmailAddress(doc["email"])
        ):
        ...
        if len(doc["login"]):
            ...
            if existing is not None:
                raise ValidationException(
                    "That login is already registered.", "login"
                )
        ...
        if (
            existing is None
            and config.getServerMode() == ServerMode.DEVELOPMENT
        ):
            doc["admin"] = True
    ...
    def filter(self, doc, user, additionalKeys=None):
        ...
        if level >= AccessType.ADMIN:
            ...
            filteredDoc["otp"] = (
                filteredDoc["otp"].get("enabled", False)
                if isinstance(filteredDoc["otp"], dict)
                else False
            )
    ...
    def authenticate(
        ...
        event = events.trigger(
            "model.user.authenticate", {"login": login, "password": password}
        )
        ...
        user = self.findOne(
            {loginField: self.hash(login), "email_encrypted": True}
        )

        if user is None and loginField == "email":
            user = self.findOne(
                {loginField: login, "email_encrypted": {"$ne": True}}
            )
    ...
    def remove(self, user, progress=None, **kwargs):
        ...
        Group().update(
            {"requests": user["_id"]}, {"$pull": {"requests": user["_id"]}}
        )

        # Delete all of the folders under this user
        folderModel = Folder()
        folders = folderModel.find(
            {"parentId": user["_id"], "parentCollection": "user"}
        )
        ...
        if progress:
            progress.update(
                increment=1, message="Deleted user " + user["login"]
            )
    ...
    def getEncryptions(self, user, email, password):
        ...
        applets = [
            AppletModel().load(ObjectId(applet_id), AccessType.READ)
        ...
        for applet in applets:
            ...
            if encryption:
                publicKey = self.getPublicKey(
                    privateKey, encryption["appletPrime"], encryption["base"]
                )
    ...
    def initializeOtp(self, user):
        ...
        from apps.girderformindlogger.api.rest import getUrlParts
        ...
        otpIssuer = (
            brandName if brandName != defaultBrandName else serverHostname
        )
    ...
    def verifyOtp(self, user, otpToken):
        lastCounterKey = (
            "girderformindlogger.models.user.%s.otp.totp.counter" % user["_id"]
        )
        ...
        try:
            ...
            raise AccessException(
                "One-time password validation failed: %s" % e
            )
    ...
    def createUser(
        ...
        if self.findOne(
            {"email": email, "email_encrypted": {"$ne": True}}
        ) or self.findOne(
            {"email": self.hash(email), "email_encrypted": True}
        ):
            raise ValidationException(
                "That email is already registered in the system.",
            )
        ...
        verifyEmail = (
            Setting().get(SettingKey.EMAIL_VERIFICATION) != "disabled"
        )
        if verifyEmail:
        ...
        self.update(
            {"_id": user["_id"]}, {"$set": {"accountId": user["accountId"]}}
        )
    ...
    def createTemplatesFolder(self, user):
        ...
        existing = Folder().findOne(
            {"accountId": user["accountId"], "meta.contentType": "templates"}
        )
        if existing:
            return existing
        ...
        return Folder().setMetadata(
            templatesFolder, {"contentType": "templates"}
        )
    ...
    def emailVerificationRequired(self, user):
        ...
        return (not user["emailVerified"]) and (
            ...
            or Setting().get(SettingKey.EMAIL_VERIFICATION) == "enabled"
        )
    ...
    def adminApprovalRequired(self, user):
        ...
        return (
            user.get("status", "enabled") == "pending"
            and Setting().get(SettingKey.REGISTRATION_POLICY) == "approve"
        )

    def _sendApprovalEmail(self, user):
        url = "%s#user/%s" % (mail_utils.getEmailUrlPrefix(), str(user["_id"]))
        text = mail_utils.renderTemplate(
            "accountApproval.mako", {"user": user, "url": url}
        )
        mail_utils.sendMailToAdmins("Girder: Account pending approval", text)

    def _sendApprovedEmail(self, user, email):
        text = mail_utils.renderTemplate(
            "accountApproved.mako",
            {"user": user, "url": mail_utils.getEmailUrlPrefix()},
        )
        mail_utils.sendMail("Girder: Account approved", text, [email])
    ...
    def _sendVerificationEmail(self, user, email):
        ...
        text = mail_utils.renderTemplate(
            "emailVerification.mako", {"url": url}
        )
    ...
    def subtreeCount(self, doc, includeItems=True, user=None, level=None):
        ...
        folders = folderModel.findWithPermissions(
            {"parentId": doc["_id"], "parentCollection": "user"},
            fields="access",
            user=user,
            level=level,
        )
    ...
    def _getGroupInvitesFromProtoUser(self, doc):
        """ """
        from apps.girderformindlogger.models.protoUser import ProtoUser
    ...
    def get_users_by_ids(self, user_ids):
        return self.find(
            query={"_id": {"$in": user_ids}}, fields=["timezone", "deviceId"]
        )
