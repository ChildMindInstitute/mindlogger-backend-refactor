...
from apps.girderformindlogger.external.notification import (
    send_push_notification,
)
from apps.girderformindlogger.models import getRedisConnection
...
class PushNotification(Scheduler):
    def __init__(self, event):
        ...
        if (
            event
            and "notifications" in event["data"]
    ...
    def set_schedules(self):
        ...
        for notification in notifications:
            ...
            try:
                ...
                self.event["sendTime"].append(
                    self.start_time.strftime("%H:%M")
                )

                if (
                    event_type == "" or event_type == "Daily"
                ):  # Daily or non-recurrent event.
    ...
    def set_reminders(self):
        ...
        if self.schedule_range["start"]:
            self.schedule_range["start"] = addDays(
                self.schedule_range["start"], reminder.get("days", 0)
            )

        if self.schedule_range["end"]:
            self.schedule_range["end"] = addDays(
                self.schedule_range["end"], reminder.get("days", 0)
            )
        ...
        if tmp <= self.current_time:
            ...
            for _ in range(4):
                ...
                if (
                    ...
                    and (tmp - self.current_time).total_seconds() / 60 > 15
                ):
    ...
    def repeat_time(self, launch_time):
        ...
        if end_time:
            end_time = datetime.strptime(
                f'{end_time} {self.start_time.strftime("%H:%M")}',
                "%Y/%m/%d %H:%M",
            )
        ...
        if end_time:
            ...
            if end_time < start_time:
                return 0
            repeats = (
                round(((end_time - start_time).total_seconds() / 3600) * 4) + 1
            )
            return repeats
        return None
    ...
    def prepare_weekly_schedule(self):
        ...
        """
        first_cron_launch = min(
            [
                (self.start_time + timedelta(minutes=15 * i)).minute
                for i in range(1, 5)
            ]
        )
        week_day = self.event.get("schedule", {}).get(
            "dayOfWeek", [self.current_time.weekday()]
        )[0]
    ...
    def prepare_monthly_schedule(self):
        ...
        dayOfMonth = self.event.get("schedule", {}).get("dayOfMonth", [None])[
            0
        ]
    ...
    def _set_scheduler_with_random_time(self):
        ...
        self.start_time = (
            self.__random_date() if self.end_time else self.start_time
        )
        self.event["sendTime"].append(self.start_time.strftime("%H:%M"))

        if (
            event_type == "" or event_type == "Daily"
        ):  # Non-recurring or daily event.
    ...
    def __set_job(
        self, first_launch=datetime.utcnow(), repeat=None, isReminder=False
    ):
        """
        ...
        job = self.schedule(
            ...
            kwargs={
                ...
                "activity_flow_id": self.event["data"].get(
    ...
    def date_format(self, notification):
        ...
        if event_type == "Weekly":
            ...
            self.schedule_range["dayOfWeek"] = schedule["dayOfWeek"][
                0
            ]  # Is this being used?
    ...
    def deleteJob(self, job):
        ...
        self.connection.delete("rq:job:" + id)
