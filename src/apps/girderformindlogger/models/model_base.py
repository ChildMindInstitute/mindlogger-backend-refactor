...
class _ModelSingleton(type):
    ...
    def __call__(cls, *args, **kwargs):
        if cls._instance is None:
            cls._instance = super(_ModelSingleton, cls).__call__(
                *args, **kwargs
            )
...
class Model(object):
    ...
    def getModelType(self, model):
        if "@type" in model:
            mt = (
                ...
                if isinstance(model["@type"], list) and len(model["@type"])
                else None
            )
            return REPROLIB_TYPES_REVERSED[mt.split(":")[-1].split("/")[-1]]
        if "meta" in model:
            for k in MODELS():
                if k in model["meta"]:
    ...
    def getFromUrl(
        ...
        modelType = (
            modelType
            if isinstance(modelType, str)
            else "external JSON-LD document"
            if modelType is None
            else modelType[0]
        )
        passedUrl = url
        ...
        if url is None:
            raise ResourcePathNotFound(
                "Document not found: {}".format(str(passedUrl))
            )

        cachedDoc = (
            cycleModels({url, passedUrl}, modelType=primary, meta=meta)[1]
            if isReloading
            else None
        )
    ...
    def validate(self, doc):
        ...
        raise NotImplementedError(
            "Must override validate() in %s model." % self.__class__.__name__
        )
        ...
        for k in keys:
            ...
            if "." in k:
                raise ValidationException(
                    'Invalid key %s: keys must not contain the "." character.'
                    % k
                )
            if k[0] == "$":
                raise ValidationException(
                    'Invalid key %s: keys must not start with the "$" character.'
                    % k
                )
    ...
    def initialize(self):
        ...
        """
        raise NotImplementedError(
            "Must override initialize() in %s model" % self.__class__.__name__
        )
    ...
    def find(
        ...
        **kwargs,
        ...
        cursor = self.collection.find(
            ...
            limit=limit,
            projection=fields,
            no_cursor_timeout=timeout is None,
            sort=sort,
            **kwargs,
        )
        if timeout:
    ...
    def _prefixSearchFilters(
        self, query, filters=None, prefixSearchFields=None
    ):
        """
        Return a set of filters and fields used in the text search.
    ...
    def save(self, document, validate=True, triggerEvents=True):
        ...
        if validate and triggerEvents:
            event = events.trigger(
                ".".join(("model", self.name, "validate")), document
            )
        ...
        try:
            if isNew:
                document["_id"] = self.collection.insert_one(
                    document
                ).inserted_id
            else:
    ...
    def remove(self, document, **kwargs):
        ...
        event = events.trigger(
            ".".join(("model", self.name, "remove")), document
        )
        kwargsEvent = events.trigger(
            ".".join(("model", self.name, "remove_with_kwargs")),
            {"document": document, "kwargs": kwargs},
        )
...
class AccessControlledModel(Model):
    ...
    def _cleanupDeletedEntity(self, event):
        ...
        if entityType == "user":
            ...
            creatorQuery = {"creatorId": entityDoc["_id"]}
            creatorUpdate = {"$set": {"creatorId": None}}
        ...
        acQuery = {"access.%ss.id" % entityType: entityDoc["_id"]}
        acUpdate = {
            "$pull": {"access.%ss" % entityType: {"id": entityDoc["_id"]}}
        }
    ...
    def _hasGroupAccessFlag(self, perms, groupIds, flag):
        ...
        for groupAccess in perms:
            if groupAccess["id"] in groupIds and flag in groupAccess.get(
                "flags", []
            ):
                return True
        return False
    ...
    def _hasUserAccessFlag(self, perms, userId, flag):
        ...
        for userAccess in perms:
            if userAccess["id"] == userId and flag in userAccess.get(
                "flags", []
            ):
                return True
        return False
    ...
    def _setAccess(
        self, doc, id, entity, level, save, flags=None, user=None, force=False
    ):
        """
        ...
        if level is not None:
            entry = {"id": id, "level": level, "flags": flags}
            entry["flags"] = self._validateFlags(
                doc, user, entity, entry, force
            )
    ...
    def _setRole(
        self, doc, id, entity, role, user=None, force=False, subject=None
    ):
        """
        Private helper for setting user roles on a resource.
        """
        from apps.girderformindlogger.models.roles import createCipher
        ...
        if role is not None:
            entry = (
                {
                    "id": id,
                    "subject": {entity: {"owl:sameAs": [id]}}
                    if ((subject is None) and (entity == "users"))
                    else {
                        k: [createCipher(s) for s in subject[k]]
                        for k in subject.keys()
                    }
                    if subject is not None
                    else None,
                }
                if USER_ROLES[role] == dict
                else {"id": id}
            )
            # because we're iterating this operation is not necessarily atomic
    ...
    def _saveAcl(self, doc, update):
        ...
        if "$set" in update:
            ...
            update["$set"] = {
                k: v for k, v in six.viewitems(doc) if k != "access"
            }
    ...
    def _saveRole(self, doc, update):
        ...
        if "$set" in update:
            ...
            update["$set"] = {
                k: v for k, v in six.viewitems(doc) if k != "roles"
            }
        event = events.trigger("model.%s.save" % self.name, doc)
        if not event.defaultPrevented:
            doc = self.collection.find_one_and_update(
    ...
    def setPublicFlags(
        self, doc, flags, user=None, append=False, save=False, force=False
    ):
        """
        ...
        if force:
            ...
            for flag in flags:
                ...
                if not info["admin"] or (
                    info["admin"] and flag in currentFlags
                ):
    ...
    def _isFlagEnabled(self, doc, type, entry, flag):
        ...
        if type == "group":
            return self._hasGroupAccessFlag(
                perms.get("groups", ()), [id], flag
            )
        else:  # 'user'
                ...
                acList["users"].append(
                    {
                        ...
                        "flags": self._validateFlags(
                            doc, user, "user", userAccess, force
                        ),
                    }
                )
            else:
                ...
                acList["groups"].append(
                    {
                        ...
                        "flags": self._validateFlags(
                            doc, user, "group", groupAccess, force
                        ),
                    }
                )
            else:
    ...
    def setGroupAccess(
        ...
        :type force: bool
        """
        return self._setAccess(
            doc, group["_id"], "groups", level, save, flags, currentUser, force
        )

    def setGroupRole(
        self, doc, group, role, currentUser=None, force=False, subject=None
    ):
        """
        ...
        return self._setRole(
            doc, group["_id"], "groups", role, currentUser, force, subject
        )
    ...
    def getAccessLevel(self, doc, user):
        ...
        """
        from apps.girderformindlogger.models.roles import checkRole
    ...
    def getFullAccessList(self, doc):
        ...
        for user in acList["users"][:]:
            userDoc = User().load(
                user["id"], force=True, fields=["firstName", "login", "email"]
            )
            if not userDoc:
                dirty = True
    ...
    def getFullRolesList(self, doc):
        ...
        """
        from apps.girderformindlogger.models.group import Group
        from apps.girderformindlogger.models.roles import decipherUser
        from apps.girderformindlogger.models.user import User
        ...
        acList = {
            role: {
                "users": doc.get("roles", {})
                .get(role, {})
                .get("users", USER_ROLES[role]()),
                "groups": doc.get("roles", {})
                .get(role, {})
                .get("groups", USER_ROLES[role]()),
            }
        ...
        for role in USER_ROLE_KEYS:
            ...
            for grp in acList[role]["groups"]:
                grpDoc = Group().load(
                    grp["id"], force=True, fields=["subject"]
                )
                if not grpDoc:
                    dirty = True
                ...
                e: [
                    {
                        "_id": str(v.get("id")),
                        "name": Group()
                        .load(v.get("id"), force=True, fields=["name"])
                        .get("name"),
                    }
                    for v in acList[k][e]
                    for v in acList[k][e]
                ]
                if USER_ROLES[k] == list
                else [
                    {
                        "_id": str(v.get("id")),
                        "name": Group()
    ...
    def setUserAccess(
        ...
        """
        return self._setAccess(
            doc, user["_id"], "users", level, save, flags, currentUser, force
        )
    ...
    def setUserRole(
        self, doc, user, role, currentUser=None, force=False, subject=None
    ):
        """
        Set group-level roles on the resource.
        ...
        return self._setRole(
            doc,
            createCipher(
                doc, self._justGetAllTheCiphers(doc), user.get("_id", user)
            ),
            "users",
            role,
            currentUser,
    ...
    def hasAccessFlags(self, doc, user=None, flags=None):
        ...
        for flag in requiredFlags:
            if not self._hasGroupAccessFlag(
                perms.get("groups", ()), user.get("groups", ()), flag
            ) and not self._hasUserAccessFlag(
    ...
    def hasAccess(self, doc, user=None, level=AccessType.READ):
        ...
        if "access" in doc:
            perms = doc["access"]
            if self._hasGroupAccess(
                perms.get("groups", []), user.get("groups", []), level
            ):
                return True
            elif self._hasUserAccess(
                perms.get("users", []), user["_id"], level
            ):
                return True
    ...
    def load(
        ...
        if isinstance(fields, six.string_types):
            logger.warning(
                "String data type for fields param is deprecated, "
        ...
        doc = Model.load(
            self, id=id, objectId=objectId, fields=loadFields, exc=exc
        )
    ...
    def filterResultsByPermission(
        self, cursor, user, level, limit=0, offset=0, removeKeys=(), flags=None
    ):
        """
        ...
        if flags:
            ...
            def hasAccess(doc):
                return self.hasAccess(
                    doc, user=user, level=level
                ) and self.hasAccessFlags(doc, user=user, flags=flags)

        else:
            hasAccess = functools.partial(
                self.hasAccess, user=user, level=level
            )
    ...
    def textSearch(
        ...
        :type level: girderformindlogger.constants.AccessType
        """
        filters, fields = self._textSearchFilters(query, filters, fields)
    ...
    def findWithPermissions(
        ...
        **kwargs,
        ...
        if level is not None:
            query = {
                "$and": [query or {}, self.permissionClauses(user, level)]
            }
        return self.find(
            query=query,
            offset=offset,
