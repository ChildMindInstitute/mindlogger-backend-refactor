...
class ResponseAlerts(AESEncryption):
    ...
    def addResponseAlerts(self, userProfile, itemId, itemSchema, alertMessage):
        ...
        if (
            "reviewer" in userProfile["roles"]
            or "manager" in userProfile["roles"]
            and userProfile["_id"] not in reviewers
        ):
        ...
        for reviewerId in reviewers:
            ...
            reviewerEmail = reviewer.get("email", "") or reviewer.get(
                "userDefined", {}
            ).get("email", "")

            if reviewerEmail and userProfile["_id"] != reviewer["_id"]:
                reviewerInfo = User().findOne(
                    {"_id": reviewer["userId"]}, fields=["lang"]
                )
                ...
                html = mail_utils.renderTemplate(
                    f"responseAlert.{lang}.mako", {"url": url}
                )

                mail_utils.sendMail("Response Alert", html, reviewerEmail)
    ...
    def getResponseAlerts(self, reviewerId, accountId):
        ...
        for alert in alerts:
            ...
            if str(alert["profileId"]) not in userProfiles:
                profile = Profile().findOne(
                    {"_id": alert["profileId"], "deactivated": {"$ne": True}}
                )
        ...
        return {
            ...
            "list": [
                alert
                for alert in alerts
                if str(alert["profileId"]) in userProfiles
            ],
        }
    ...
    def deleteResponseAlerts(self, profileId):
        self.removeWithQuery(query={"profileId": ObjectId(profileId)})
