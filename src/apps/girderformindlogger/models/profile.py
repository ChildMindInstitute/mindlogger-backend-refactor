...
class Profile(AESEncryption, dict):
    ...
    def display(self, p, role):
        ...
        prof = {
            k: v
            for k, v in p.get("coordinatorDefined", {}).items()
            if (v is not None and (role is not "user" or k is not "email"))
        }
        prof.update(
            {
                k: v
                for k, v in p.get("userDefined", {}).items()
                if (v is not None and (role is not "user" or k is not "email"))
            }
        )
        return prof
    ...
    def load(
        ...
        if doc is not None:
            for key in ["coordinatorDefined", "userDefined"]:
                if key not in doc:
                    doc[key] = {}
                    self.update({"_id": doc["_id"]}, {"$set": {key: {}}})
    ...
    def coordinatorProfile(self, appletId, coordinator):
        ...
        if isinstance(coordinator, dict) and "userId" not in coordinator:
            coordinator = self.createProfile(
                appletId, coordinator, "coordinator"
            )
        return (
            self.cycleDefinitions(coordinator, showEmail=True)
            if Applet().isCoordinator(appletId, coordinator)
            else {}
        )
        ...
        if isinstance(user, dict):
            ...
            return (
                ...
                if user is not None
                    ...
                    User().load(str(profile["userId"]), force=True)
                    if (isinstance(profile, dict) and "userId" in profile)
                    else {}
                )
            )
        if isinstance(user, str):
            try:
                return self._canonicalUser(
                    appletId, User().load(user, force=True)
                )
            except:
                return self._canonicalUser(
                    appletId, self.load(user, force=True)
                )
    ...
    def cycleDefinitions(self, userProfile, showEmail=False, showIDCode=False):
        ...
        if showIDCode:
            ...
            if userProfile.get("profile", False):
                displayProfile.update(
                    {"idCodes": IDCode().findIdCodes(userProfile["_id"])}
                )
            if userProfile.get("code", False):
                displayProfile.update({"idCode": userProfile.get("code")})

        return {
            k: v if v != "" else None
            for k, v in displayProfile.items()
    ...
    def profileAsUser(self, profile, requester):
        ...
        return self.cycleDefinitions(
            ...
            showEmail=any(
                [
                    AppletModel().isCoordinator(
                        profile["appletId"], requester
                    ),
            ...
            showIDCode=any(
                [
                    ...
                    AppletModel()._hasRole(
                        profile["appletId"], requester, "reviewer"
                    ),
                ]
            ),
        )
    ...
    def displayProfileFields(
        self, profile, user=None, forceManager=False, forceReviewer=False
    ):
        """
    ...
    def _cacheProfileDisplay(
        self, profile, user, forceManager=False, forceReviewer=False
    ):
        from apps.girderformindlogger.models.applet import Applet

        profileDefinitions = self.cycleDefinitions(
            profile,
            showEmail=forceManager
            if forceManager
            else Applet().isCoordinator(profile["appletId"], user),
            showIDCode=forceReviewer
            if forceReviewer
            else Applet().isCoordinator(profile["appletId"], user),
        )
    ...
    def getSubjectProfile(self, id, displayName, user):
        ...
        if ps is not None:
            ...
            ps = [
                p
                for p in ps
                if p is not None and p.get("displayName") == displayName
            ]
            if len(ps):
                return ps[0]
        else:
    ...
    def updateProfiles(self, user, data):
        ...
        try:
            self.update(
                query={"userId": {"$in": [user["_id"]]}, "profile": True},
                update=data,
                multi=True,
            )
    ...
    def _updateDescendants(self, folderId, updateQuery):
        ...
        self.update(
            query={"appletId": folderId, "parentCollection": "profile"},
            update=updateQuery,
            multi=True,
        )
        Item().update(
    ...
    def remove(self, folder, progress=None, **kwargs):
        ...
        if progress:
            progress.update(
                increment=1, message="Deleted profile %s" % folder["name"]
            )
    ...
    def childFolders(
        ...
        **kwargs,
        ...
        q = {"appletId": parent["_id"], "parentCollection": parentType}
        q.update(filters)
    ...
    def createFolder(
        ...
        if creator is not None:
            self.setUserAccess(
                folder, user=creator, level=AccessType.ADMIN, save=False
            )
    ...
    def getProfileData(self, profile, viewer):
        ...
        data = {field: profile.get(field, "") for field in fields}
        ...
        if "firstName" in fields and not data["firstName"]:
            data["firstName"] = profile.get("userDefined", {}).get(
                "displayName", ""
            )

        # reviewers don't need to view user's roles
        if (
            "coordinator" not in viewer["roles"]
            and "manager" not in viewer["roles"]
        ):
        ...
        if "coordinator" in viewer["roles"]:
            data["hasIndividualEvent"] = (
                profile.get("individual_events", 0) > 0
            )

        data["viewable"] = False
        if (
            viewer["_id"] in profile.get("reviewers", [])
            or viewer["_id"] == profile["_id"]
        ):
        ...
        for userActivityUpdate in profile.get("completed_activities", []):
            if userActivityUpdate["completed_time"] and (
                not data["updated"]
                or data["updated"] < userActivityUpdate["completed_time"]
            ):
                data["updated"] = userActivityUpdate["completed_time"]

        for userFlowUpdate in profile.get("activity_flows", []):
            if userFlowUpdate["completed_time"] and (
                not data["updated"]
                or data["updated"] < userFlowUpdate["completed_time"]
            ):
    ...
    def generateMissing(self, applet):
        ...
        userList = {
            str(ulu["user"]["_id"]): {
                k: v
                for k, v in {
                    "displayName": ulu["user"]
                    .get("coordinatorDefined", {})
                    .get(
                        "displayName",
                        ulu["user"]
                        .get("userDefined", {})
    ...
    def updateOwnerProfile(self, applet, invitationId=None):
        ...
        appletProfile = self.findOne(
            {"userId": owner["userId"], "appletId": applet["_id"]}
        )
    ...
    def updateReviewerList(
        self, reviewer, users=None, operation="replace", isMRNList=False
    ):
        ...
        for profile in profiles:
            ...
            if operation == "delete":  # delete
                if (
                    users
                    and profile["_id"] in users
                    and reviewer["_id"] in profile["reviewers"]
                ):
                    profile["reviewers"].remove(reviewer["_id"])
                    self.update(
                        {"_id": profile["_id"]},
                        {"$set": {"reviewers": profile["reviewers"]}},
                    )
    ...
    def getReviewerListForUser(self, appletId, userProfile, user):
        ...
        for reviewer in userProfile["reviewers"]:
            reviewers.append(
                self.displayProfileFields(
                    self.findOne({"_id": reviewer}), user, forceManager=True
                )
            )
    ...
    def createProfile(self, applet, user, role="user"):
        ...
        if applet["_id"] not in [
            ...
            roles = (
                ["user", "editor", "reviewer", "coordinator", "manager"]
                if role == "manager"
                else [role]
            )
            ...
            for role in roles:
                ...
                if bool(groups):
                    group = Group().load(
                        ObjectId(list(groups.keys())[0]), force=True
                    )
        ...
        managers = list(
            self.find(
                query={"appletId": applet["_id"], "roles": "manager"},
                fields=["_id"],
            )
        )
        profile = {
            ...
            for k, v in {
                ...
                "completed_activities": [
                    ...
                    for activity_id in applet.get("meta", {})
                    .get("protocol", {})
                    .get("activities", [])
                ],
                "activity_flows": [
                    ...
                    for activity_flow_id in applet.get("meta", {})
                    .get("protocol", {})
                    .get("activityFlows", [])
                ],
                "accountId": applet.get("accountId", None),
                ...
                "userDefined": {
                    ...
                    if not user.get("email_encrypted", None)
                    else "",
                },
    ...
    def createFakeProfile(self, applet):
        profile = self.findOne(
            {"appletId": ObjectId(applet["_id"]), "fake": True}
        )
    ...
    def createPassiveProfile(self, appletId, code, displayName, coordinator):
        ...
        profile = {
            ...
            for k, v in {
                ...
                "size": 0,
                "coordinatorDefined": {"displayName": displayName},
                "createdBy": self.coordinatorProfile(appletId, coordinator),
            }.items()
            if v is not None
        }
        ...
        return {
            k: v
            for k, v in self.load(profile["_id"], force=True).items()
    ...
    def countFolders(self, folder, user=None, level=None):
        ...
        folders = self.findWithPermissions(
            {"appletId": folder["_id"], "parentCollection": "profile"},
            fields=fields,
            user=user,
            level=level,
        )
    ...
    def subtreeCount(self, folder, includeItems=True, user=None, level=None):
        ...
        count += sum(
            self.subtreeCount(
                subfolder, includeItems=includeItems, user=user, level=level
            )
    ...
    def setAccessList(
        ...
        if recurse:
            subfolders = self.findWithPermissions(
                {"appletId": doc["_id"], "parentCollection": "profile"},
                user=user,
                level=AccessType.ADMIN,
            )
    ...
    def get_profiles_by_user_ids(self, user_ids):
        return self.find(query={"userId": {"$in": user_ids}, "profile": True})
