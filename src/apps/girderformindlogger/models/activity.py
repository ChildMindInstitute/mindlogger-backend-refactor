...
class Activity(Folder):
    ...
    def listVersionId(
        ...
        if doc is not None:
            ...
            try:
                if (
                    baseParent["object"]["name"].lower() == "activities"
                    and doc["baseParentType"] == "collection"
                ):
                    return [str(doc["_id"]) if "_id" in doc else str(id)]
                parent = pathFromRoot[-1]["object"]
                grandparent = pathFromRoot[-2]["object"]
                if grandparent["lowerName"] == "applets" and parent[
                    "baseParentType"
                ] in {"collection", "user"}:
                    """
                ...
                if greatgrandparent["lowerName"] == "applets" and parent[
                    "baseParentType"
                ] in {"collection", "user"}:
                    """
                ...
                raise ValidationException("Invalid Activity ID.")
    ...
    def disableConditionals(self, activity):
        ...
        for itemProperty in properties:
            itemProperty["reprolib:terms/isVis"] = [{"@value": True}]
