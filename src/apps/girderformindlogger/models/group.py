...
class Group(AccessControlledModel):
    ...
    def validate(self, doc):
        ...
        if duplicate is not None:
            raise ValidationException(
                "A group with that name already exists.", field="name"
            )
    ...
    def listMembers(self, group, offset=0, limit=0, sort=None):
        ...
        return User().find(
            {"groups": group["_id"]}, limit=limit, offset=offset, sort=sort
        )
    ...
    def remove(self, group, **kwargs):
        ...
        User().update(
            {"groups": group["_id"]}, {"$pull": {"groups": group["_id"]}}
        )
    ...
    def getMembers(self, group, offset=0, limit=0, sort=None):
        ...
        return User().find(
            {"groups": group["_id"]}, offset=offset, limit=limit, sort=sort
        )
    ...
    def removeUser(self, group, user, delete=False):
        ...
        if delete:
            ...
            [
                ...
                for rf in list(
                    set(
                        responseFolder["parentId"]
                        for responseFolder in [
                            Folder().load(
                                folder,
                                        ...
                                        Item().find(
                                            query={
                                                "meta.applet.@id": {
                                                    "$in": [
                                                        applet["_id"]
                                                        for applet in Applet().getAppletsForGroup(
                                                            "user",
                                                            group.get(
                                                                "_id", group
                                                            ),
    ...
    def getFullRequestList(self, group):
        ...
        for userId in group.get("requests", []):
            user = userModel.load(
                userId, force=True, fields=["firstName", "login"]
            )
            yield {
                "id": userId,
                "login": user["login"],
    ...
    def hasAccess(self, doc, user=None, level=AccessType.READ):
        ...
        from apps.girderformindlogger.constants import USER_ROLES
        from apps.girderformindlogger.models.applet import Applet
        from apps.girderformindlogger.models.user import User
        ...
        if user is None:
            ...
            return (
                level == AccessType.READ and doc.get("public", False) is True
            )
            ...
            return (
                doc.get("public", False) is True
                or doc["_id"] in user.get("groups", [])
                or doc["_id"]
                ...
                [
                    {prefix + "_id": {"$in": user.get("groups", [])}},
                    {
                        ...
                        + "_id": {
                            "$in": [
                                i["groupId"]
                                for i in user.get("groupInvites", [])
                            ]
                        }
                    },
                ]
