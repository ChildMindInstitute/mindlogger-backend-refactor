...
from apps.girderformindlogger.utility import path as path_util
...
class File(acl_mixin.AccessControlMixin, Model):
    ...
    def initialize(self):
        ...
        self.exposeFields(
            level=AccessType.SITE_ADMIN, fields=("assetstoreId",)
        )

        events.bind(
            "model.file.save.created",
    ...
    def _getAssetstoreModel(self, file):
        from apps.girderformindlogger.models.assetstore import Assetstore
    ...
    def createLinkFile(
        ...
        :type parent: girderformindlogger.models.folder or girderformindlogger.models.item
        ...
        existing = None
        if reuseExisting:
            existing = self.findOne({"itemId": item["_id"], "name": name})
        ...
        file.update(
            {"creatorId": creator["_id"], "mimeType": mimeType, "linkUrl": url}
        )
        if size is not None:
            file["size"] = int(size)
    ...
    def open(self, file):
        ...
        :rtype: girderformindlogger.utility.abstract_assetstore_adapter.FileHandle
        """
        return self.getAssetstoreAdapter(file).open(file)

    def getGirderMountFilePath(self, file, validate=True):
        """
        If possible, get the path of the file on a local girderformindlogger mount.

        :param file: The file document.
        :param validate: if True, check if the path exists and raise an
            exception if it does not.
        :returns: a girderformindlogger mount path to the file or None if no such path is
            available.
        """
        ...
        if validate:
            raise FilePathException(
                "This file isn't accessible from a Girder mount."
            )
