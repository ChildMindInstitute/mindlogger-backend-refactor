...
class Folder(AccessControlledModel):
    ...
    def load(
        ...
        if doc is not None:
            ...
            if "meta" not in doc:
                doc["meta"] = {}
                self.update({"_id": doc["_id"]}, {"$set": {"meta": {}}})
    ...
    def getSizeRecursive(self, folder):
        ...
        q = {"parentId": folder["_id"], "parentCollection": "folder"}
    ...
    def _updateDescendants(self, folderId, updateQuery):
        ...
        self.update(
            query={"parentId": folderId, "parentCollection": "folder"},
            update=updateQuery,
            multi=True,
        )
        Item().update(
        ...
        q = {"parentId": folderId, "parentCollection": "folder"}
        for child in self.find(q):
    ...
    def move(self, folder, parent, parentType):
        ...
        if (folder["baseParentType"], folder["baseParentId"]) != (
            ...
            self._updateDescendants(
                folder["_id"],
                {"$set": {"baseParentType": rootType, "baseParentId": rootId}},
            )
    ...
    def clean(self, folder, progress=None, **kwargs):
        ...
        for item in items:
            ...
            if progress:
                progress.update(
                    increment=1, message="Deleted item %s" % item["name"]
                )
    ...
    def remove(self, folder, progress=None, **kwargs):
        ...
        uploads = uploadModel.find(
            {"parentId": folder["_id"], "parentType": "folder"}
        )
        ...
        if progress:
            progress.update(
                increment=1, message="Deleted folder %s" % folder["name"]
            )
    ...
    def filter(self, doc, user=None, additionalKeys=None):
        ...
        """
        filteredDoc = super(Folder, self).filter(
            doc, user, additionalKeys=additionalKeys
        )
    ...
    def parentsToRoot(
        ...
        if curParentType in ("user", "collection"):
            ...
            if force:
                ...
                parentFiltered = ModelImporter.model(curParentType).filter(
                    curParentObject, user
                )

            return [
                {"type": curParentType, "object": parentFiltered}
            ] + curPath
        else:
            ...
            curPath = [
                {
                    "type": curParentType,
                    "object": curParentObject
                    if force
                    else self.filter(curParentObject, user),
                }
            ] + curPath

            return self.parentsToRoot(
                curParentObject, curPath, user=user, force=force
            )
    ...
    def copyFolderComponents(
        self, srcFolder, newFolder, creator, progress, firstFolder=None
    ):
        """
        ...
        for item in self.childItems(folder=srcFolder):
            ...
            if progress:
                progress.update(
                    increment=1, message="Copied item " + item["name"]
                )
        # copy subfolders
        for sub in self.childFolders(
            parentType="folder", parent=srcFolder, user=creator
        ):
        ...
        if progress:
            progress.update(
                increment=1, message="Copied folder " + newFolder["name"]
            )
    ...
    def setAccessList(
        ...
        if recurse:
            subfolders = self.findWithPermissions(
                {"parentId": doc["_id"], "parentCollection": "folder"},
                user=user,
                level=AccessType.ADMIN,
            )
