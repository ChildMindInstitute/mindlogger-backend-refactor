...
class Events(Model):
        ...
        if event:
            if event["individualized"]:
                ProfileModel().update(
                    ...
                    update={
                        "$inc": {"individual_events": -1},
                        "$unset": {f"finished_events.{str(event_id)}": ""},
                    },
                )
            else:
                ProfileModel().update(
                    query={"appletId": event["applet_id"]},
                    update={
                        "$unset": {f"finished_events.{str(event_id)}": ""}
                    },
                )
        ...
        events = self.find(
            {
                ...
                "data.activity_id": ObjectId(activity_id),
            }
        )
    ...
    def deleteEventsByActivityFlowId(self, applet_id, activity_flow_id):
        events = self.find(
            {
                "applet_id": ObjectId(applet_id),
                "data.activity_flow_id": ObjectId(activity_flow_id),
            }
        )
    ...
    def upsertEvent(self, event, applet, event_id=None):
        ...
        existed_event = self.findOne(
            {"_id": ObjectId(event_id)}, fields=["_id", "schedulers", "data"]
        )
        ...
        if "data" in event:
            ...
            if "activity_id" in newEvent["data"]:
                newEvent["data"]["activity_id"] = ObjectId(
                    newEvent["data"]["activity_id"]
                )

            if "activity_flow_id" in newEvent["data"]:
                newEvent["data"]["activity_flow_id"] = ObjectId(
                    newEvent["data"]["activity_flow_id"]
                )

            if "users" in event["data"] and isinstance(
                event["data"]["users"], list
            ):
                newEvent["individualized"] = True
                event["data"]["users"] = [
                    ObjectId(profile_id)
                    for profile_id in event["data"]["users"]
                ]

                self.updateIndividualSchedulesParameter(
                    newEvent, existed_event
                )
    ...
    def notify_user_about_event_changes(
        self, event_list, applet, previous_device_ids=None
    ):
        ...
        if event_list:
            for event in event_list:
                if event["data"].get("users"):
                    device_ids.update(
                        self.get_applet_device_ids(
                            applet["_id"], event["data"]["users"]
                        )
                    )
                else:
                    device_ids.update(
                        self.get_applet_device_ids(applet["_id"])
                    )
        else:
    ...
    def get_applet_device_ids(applet_id, users=None) -> list:
        if not users:
            profiles = ProfileModel().find(
                query=dict(appletId=applet_id, profile=True),
                fields=["deviceId"],
            )
        else:
            profiles = ProfileModel().find(
                query=dict(
                    appletId=applet_id, profile=True, _id={"$in": users}
                ),
                fields=["deviceId"],
            )
    ...
    def updateIndividualSchedulesParameter(self, newEvent, oldEvent):
        ...
        if len(dicrementedUsers):
            Profile().update(
                query={"_id": {"$in": dicrementedUsers}},
                update={"$inc": {"individual_events": -1}},
            )

        if len(incrementedUsers):
            Profile().update(
                query={"_id": {"$in": incrementedUsers}},
                update={"$inc": {"individual_events": 1}},
            )
    ...
    def dateMatch(
        self, event, date
    ):  # filter only active events on specified date
        ...
        timeDelta = datetime.timedelta(
            hours=int(eventTime[:2]), minutes=int(eventTime[-2:])
        )
        ...
        if (
            eventTimeout
            and eventTimeout.get("allow", False)
            and event["data"].get("completion", False)
        ):
        ...
        if (
            not event["data"].get("eventType", None)
            or event["data"]["eventType"] == "onetime"
        ):  # one time schedule
            if (
                not len(event["schedule"].get("dayOfMonth", []))
            ...
            startDate = (
                datetime.datetime.fromtimestamp(start / 1000) + timeDelta
                if start
                else None
            )
            endDate = (
                datetime.datetime.fromtimestamp(end / 1000) + timeDelta
                if end
                else None
            )
            ...
            if (
                event["data"].get("eventType", None) == "Weekly"
            ):  # weekly schedule
                if (
                    len(event["schedule"]["dayOfWeek"])
                    and event["schedule"]["dayOfWeek"][0]
                    == (date.weekday() + 1) % 7
                ):
                    return (True, None)
                ...
                if (
                    not startDate
                    or startDate.date() <= latestScheduledDay.date()
                ):
                    lastAvailableTime = (
                        latestScheduledDay + timeDelta + timeout
                    )
                ...
                event["data"].get("eventType", None) == "Monthly"
            ):  # monthly schedule
                if (
                    len(event["schedule"]["dayOfMonth"])
                    and event["schedule"]["dayOfMonth"][0] == date.day
                ):
                    return (True, None)

                if endDate and endDate < date:
                    latestScheduledDay = datetime.datetime(
                        endDate.year,
                        endDate.month,
                    ...
                    if endDate.day < event["schedule"]["dayOfMonth"][0]:
                        latestScheduledDay = (
                            latestScheduledDay - relativedelta(months=1)
                        )
                else:
                    month = date.month
                    ...
                    latestScheduledDay = datetime.datetime(
                        date.year, month, event["schedule"]["dayOfMonth"][0]
                    )
            ...
            # daily schedule
            lastAvailableTime = (
                endDate + timeDelta + timeout if endDate else None
            )
            return (
                (not endDate or lastAvailableTime >= date),
                lastAvailableTime,
            )
    ...
    def getScheduleForUser(self, applet_id, user_id, eventFilter=None):
        profile = Profile().findOne(
            {"appletId": ObjectId(applet_id), "userId": ObjectId(user_id)}
        )
        result = {"events": {}}
            ...
            if eventFilter:
                ...
                for i in range(0, eventFilter[1]):
                    ...
                    for event in duplicate_events:
                        ...
                        if not event["valid"]:
                            if lastAvailableTime:
                                if identifier not in lastEvent or (
                                    lastEvent[identifier]
                                    and lastAvailableTime
                    ...
                    for value in lastEvent.values():
                        if value and (
                            value[1]["data"].get("completion", False)
                            or not value[1]["data"].get("availability", False)
                        ):
                            data.append(value[1])
                    ...
                    result["data"][dayFilter.strftime("%Y/%m/%d")] = [
                        {"id": str(card["id"]), "valid": card["valid"]}
                        for card in data
                    ]
