...
from apps.girderformindlogger.constants import AccessType, TokenScope
from apps.girderformindlogger.exceptions import AccessException
...
from apps.girderformindlogger.utility import genToken
...
class Token(AccessControlledModel):
    ...
    def createToken(
        self, user=None, days=None, scope=None, apiKey=None, accountId=None
    ):
        """
        ...
        if user is None:
            ...
            token["accountId"] = (
                accountId if accountId else user.get("accountId", None)
            )
            self.setUserAccess(
                token, user=user, level=AccessType.ADMIN, save=False
            )
