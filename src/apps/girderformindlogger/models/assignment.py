...
class Assignment(Folder):
    ...
    def create(self, applet, user):
        ...
        try:
            ...
            raise ValidationException(
                message="Invalid Applet ID", field="applet"
            )
        ...
        try:
            assignment = Folder().createFolder(
                parent=assignmentsCollection,
                parentType="collection",
                name=Applet().preferredName(applet),
                creator=user,
                reuseExisting=True,
                public=False,
            )
    ...
    def findAssignments(self, applet):
        ...
        allAssignments = [
            {"_id": a["_id"], "_modelType": "collection"}
            for a in Collection().find({"name": "Assignments"})
        ] + [
            {"_id": a["_id"], "_modelType": "folder"}
    ...
    def load(
        ...
        if doc is not None:
            ...
            try:
                ...
                raise ValidationException("Invalid Assignment Applet ID.")
