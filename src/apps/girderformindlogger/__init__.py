...
class LogFormatter(logging.Formatter):
    ...
    def formatException(self, exc):
        info = "\n".join(
            (
                "  Request URL: %s %s"
                % (cherrypy.request.method.upper(), cherrypy.url()),
                "  Query string: " + cherrypy.request.query_string,
                "  Remote IP: " + cherrypy.request.remote.ip,
                "  Request UID: "
                + getattr(cherrypy.request, "girderRequestUid", "[none]"),
            )
        )
        return (
            "%s\n"
    ...
    def __init__(self, stream, logger, level):
        ...
        for key in dir(stream):
            ...
            if (
                ...
                and (
                    callable(getattr(stream, key, None))
                    or isinstance(
...
def _attachFileLogHandlers():
    ...
    if isinstance(
        getattr(logging, logCfg.get("log_max_info_level", ""), None), int
    ):
...
def _setupCache():
    ...
    if curConfig["cache"]["enabled"]:
        ...
        requestCache.configure_from_config(
            curConfig["cache"], "cache.request."
        )
    else:
