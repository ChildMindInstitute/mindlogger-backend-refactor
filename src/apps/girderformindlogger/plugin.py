"""
This module defines functions for registering, loading, and querying girderformindlogger plugins.
"""
...
_NAMESPACE = "girderformindlogger.plugin"
...
class GirderPlugin(object):
    """
    This is a base class for describing a girderformindlogger plugin.  A plugin is registered by adding
    an entrypoint under the namespace ``girderformindlogger.plugin``.  This entrypoint should return a
    ...
    Example ::
        class Cats(GirderPlugin):
            ...
            def load(self, info):
                ...
                girderformindlogger.plugin.getPlugin('pets').load(info)
    ...
    def npmPackages(self):
        ...
        if not os.path.isfile(packageJsonFile):
            raise Exception(
                "Invalid web client path provided: %s" % packageJsonFile
            )
