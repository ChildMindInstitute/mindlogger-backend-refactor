...
def formatUserString(u):
    ...
    return "{name} {email}".format(
        ...
        email="({})".format(u["email"])
        if "email" in u
        and u["email"] != ""
        and not u.get("email_encrypted", None)
        else "",
    ).strip()
...
_templateLookup = TemplateLookup(
    directories=[_templateDir], collection_size=50
)
...
class _SMTPConnection(object):
    def __init__(
        self, host, port=None, encryption=None, username=None, password=None
    ):
        self.host = host
        self.port = port
...
def _submitEmail(msg, recipients):
    ...
    logger.info(
        "Sending email to %s through %s", ", ".join(recipients), smtp.host
    )
...
def sendMail(subject, text, to, bcc=None, attachments=[]):
    ...
    events.daemon.trigger(
        "_sendmail", info={"message": msg, "recipients": recipients}
    )
