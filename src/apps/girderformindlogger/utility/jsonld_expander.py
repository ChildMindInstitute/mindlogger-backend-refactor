...
def convertObjectToSingleFileFormat(
    obj, modelType, user, identifier=None, refreshCache=False, modelClasses={}
):
    ...
    obj.update({"loadedFromSingleFile": True, "lastUpdatedBy": user["_id"]})
    ...
    if refreshCache:
        ...
        formatted = formatLdObject(
            obj, mesoPrefix=modelType, user=user, refreshCache=True
        )
...
def insertHistoryData(
    ...
    if not referenceObj:
        referenceObj = itemModel.setMetadata(
            ...
            {"identifier": identifier, "modelType": modelType, "history": []},
        )

    now = datetime.utcnow()

    itemModel.update(
        {"_id": referenceObj["_id"]},
        {
            "$push": {
                "meta.history": {
                    "version": baseVersion,
                    "reference": "{}/{}".format(modelType, str(obj["_id"]))
                    if obj
                    else None,
                    "updated": now,
                }
            },
            "$set": {"updated": now, "meta.lastVersion": baseVersion},
        },
    )
...
def createProtocolFromExpandedDocument(
    protocol, user, editExisting=False, removed={}, baseVersion=None
):
    protocolId = None
    historyFolder = None
        ...
        for model in protocol[modelType].values():
            ...
            if modelClass.name in ["folder", "item"]:
                ...
                if model["ref2Document"].get("_id", None) and editExisting:
                    ...
                    if modelClass.name == "folder":
                        try:
                            docFolder = FolderModel().load(
                                model["ref2Document"]["_id"], force=True
                            )

                            if "identifier" in docFolder["meta"] and (
                                modelType == "activity"
                                or modelType == "activityFlow"
                            ):
                                model["historyObj"] = insertHistoryData(
                                    deepcopy(docFolder),
                                    docFolder["meta"]["identifier"],
                                    modelType,
                                    baseVersion,
                                    historyFolder,
                                    historyReferenceFolder,
                            ...
                            print(
                                "wrong folder id", model["ref2Document"]["_id"]
                            )
                            print(e)
                    elif modelClass.name == "item":
                        try:
                            item = modelClass.load(
                                model["ref2Document"]["_id"], force=True
                            )
                            ...
                            if "identifier" in item["meta"]:
                                ...
                                if "activityId" in clonedItem["meta"]:
                                    clonedItem["meta"].update(
                                        {
                                            "originalActivityId": clonedItem[
                                                "meta"
                                            ]["activityId"],
                                            "activityId": protocol[
                                                model["parentKey"]
                                            ][model["parentId"]]["historyObj"][
                                                "_id"
                                            ],
                                        }
                                    )
                            ...
                            print(
                                "wrong item id", model["ref2Document"]["_id"]
                            )
                            print(e)
                ...
                if modelClass.name == "folder":
                    ...
                    if item:
                        ...
                        metadata["identifier"] = "{}/{}".format(
                            metadata["activityId"], str(item["_id"])
                        )
                ...
                if "duplicateOf" in model["ref2Document"]:
                    update["duplicateOf"] = ObjectId(
                        model["ref2Document"]["duplicateOf"]
                    )

                newModel.update(update)

                modelClass.update({"_id": newModel["_id"]}, {"$set": update})
                ...
                if modelType == "protocol":
                    ...
                    if (
                        editExisting
                        and baseVersion
                        and newModel.get("meta", {}).get("historyId", None)
                    ):
                        historyFolder = FolderModel().load(
                            newModel["meta"]["historyId"], force=True
                        )
                        historyReferenceFolder = FolderModel().load(
                        ...
                        if "items" in removed:
                            ...
                            for item in removedItems:
                                ...
                                if "identifier" in item["meta"]:
                                    ...
                                    historyObj = activityIdToHistoryObj.get(
                                        activityId, None
                                    )
    ...
    for modelType in ["screen", "activity", "activityFlow"]:
        for model in models[modelType]:
            formatted = _fixUpFormat(
                formatLdObject(
                    model, mesoPrefix=modelType, user=user, refreshCache=True
                )
            )
...
def getUpdatedContent(updates, document):
    ...
    activityID2Key = {
        str(activityUpdates[key]["data"]["_id"]): key
        for key in activityUpdates
    }
...
def cacheProtocolContent(protocol, document, user, editExisting=False):
    ...
    if protocol.get("meta", {}).get("contentId", None):
        contentFolder = FolderModel().load(
            protocol["meta"]["contentId"], force=True
        )
        ...
        if editExisting and "baseVersion" in document:
            ...
            for key in dict.keys(latestDocument["protocol"]["activities"]):
                ...
                if type(activity) == str:
                    latestDocument["protocol"]["activities"][key] = loadCache(
                        activity.split("/").pop()
                    )
            ...
            for key in dict.keys(newContent["protocol"]["activities"]):
                if type(newContent["protocol"]["activities"][key]) == dict:
                    ...
                    newContent["protocol"]["activities"][
                        key
                    ] = f'cache/{str(cached["_id"])}'
            ...
            if type(document) == dict:
                ...
                document.seek(0)
                for key, activity in ijson.kvitems(
                    document, "protocol.activities"
                ):
                    content["protocol"]["activities"][key] = activity

                document.seek(0)
                for key, activityFlow in ijson.kvitems(
                    document, "protocol.activityFlows"
                ):
            ...
            item["length"] = len(
                json_util.dumps(content, default=decimal_default)
            )
            for key in list(
                dict.keys(content["protocol"].get("activities", {}))
            ):
                cached = cacheModel.insertCache(
                    "item",
                    item["_id"],
                    "content",
                    content["protocol"]["activities"][key],
                )
                content["protocol"]["activities"][
                    key
                ] = f'cache/{str(cached["_id"])}'
...
def updateContributions(protocol, document, user):
    ...
    if protocol.get("meta", {}).get("contributionId", None):
        contributionFolder = FolderModel().load(
            protocol["meta"]["contributionId"], force=True
        )
        ...
        if "items" in activity:
            for item in activity["items"].values():
                if item.get("baseItemId", None) and item.get(
                    ...
                    baseItem = ScreenModel().load(
                        item["baseItemId"], force=True
                    )
                    contribution["meta"] = {
                        ...
                        "lastUpdatedBy": baseItem.get(
                            "lastUpdatedBy", baseItem.get("creatorId")
                        ),
                    }
                    ...
                    if appletId not in appletIdToAccountId:
                        ...
                        formatted = formatLdObject(
                            applet, "applet", None, refreshCache=False
                        )

                        appletIdToVersion[appletId] = (
                            formatted["applet"]
                            .get("schema:schemaVersion", [{}])[0]
                            .get("@value", "0.0.0")
                        )
                    ...
                    contribution["meta"][
                        "baseAccountId"
                    ] = appletIdToAccountId[appletId]
                    contribution["meta"]["version"] = appletIdToVersion[
                        appletId
                    ]
                    ...
                    if baseItem.get("cached", None):
                        ...
                        cache = _fixUpFormat(
                            formatLdObject(
                                baseItem, mesoPrefix="screen", user=user
                            )
                        )
...
def loadFromSingleFile(document, user, editExisting=False):
    ...
    for activity in document["protocol"].get("activities", {}).values():
        ...
        if "items" in activity:
            for item in activity["items"].values():
                ...
                protocol["screen"][
                    "{}.{}".format(
                        expandedActivity["@id"], expandedItem["@id"]
                    )
                ] = {
                    "parentKey": "activity",
    ...
    protocolId = createProtocolFromExpandedDocument(
        protocol,
        user,
        editExisting,
        document.get("removed", {}),
        document.get("baseVersion", None),
    )
    ...
    return formatLdObject(
        protocol, mesoPrefix="protocol", user=user, refreshCache=True
    )


def importAndCompareModelType(
    model, url, user, modelType, meta={}, existing=None
):
    import threading

    from apps.girderformindlogger.utility import firstLower
    ...
    modelType = (
        "screen"
        if modelType.lower() == "field"
        else "protocol"
        if modelType.lower() == "activityset"
        else modelType
    )
    changedModel = (atType != modelType and len(atType)) or (" " in modelType)
    ...
    if modelClass.name in ["folder", "item"]:
        ...
        if modelClass.name == "folder":
            newModel = modelClass.setMetadata(
                docFolder,
                {
                    modelType: {**model, "schema:url": url, "url": url},
                    **meta,
                    "schema": APPLET_SCHEMA_VERSION,
                },
            )
        ...
        modelClass.update(
            {"_id": newModel["_id"]}, {"$set": {"loadedFromSingleFile": False}}
        )
        newModel["loadedFromSingleFile"] = False

    formatted = _fixUpFormat(
        formatLdObject(
            newModel, mesoPrefix=modelType, user=user, refreshCache=True
        )
    )
...
def _createContextForStr(s):
    sp = s.split("/")
    k = (
        "_".join(sp[:-1] if "." not in sp[-1] else sp)
        .replace(".", "")
        .replace(":", "")
    )
    return (
        ({k: "{}/".format("/".join(sp[:-1]))}, "{}:{}".format(k, sp[-1]))
        if "." not in sp[-1]
        else ({k: s}, k)
    )
...
def contextualize(ldObj):
    ...
    if isinstance(context, list):
        context.append({"reprolib": REPROLIB_CANONICAL})
    elif isinstance(context, dict):
        context["reprolib"] = REPROLIB_CANONICAL
    for k in ldObj.keys():
        if isinstance(ldObj[k], dict):
            context, newObj[k] = _deeperContextualize(ldObj[k], context)
        else:
            newObj[k] = ldObj[k]
...
def childByParent(parent, applet, parentProfile=None):
    ...
    parentProfile = (
        Profile().getProfile(
            Profile()
            .createProfile(
                applet["applet"]["_id"].split("applet/")[-1], parent, "user"
            )
            .get("_id"),
            user=parent,
        )
        if parentProfile is None
        else parentProfile
    )
    parentKnows = parentProfile.get("schema:knows", {})
    children = [
        Profile().displayProfileFields(Profile().load(p, force=True), parent)
        for p in list(
    ...
    return [formatChildApplet(child, deepcopy(applet)) for child in children]


def formatChildApplet(child, applet):
    applet["applet"] = _formatChildLabel(
        applet["applet"], child["displayName"]
    )
    for act in applet["activities"]:
        applet["activities"][act] = _formatChildLabel(
            applet["activities"][act], child["displayName"]
        )
...
def _formatChildLabel(obj, label):
    for i, pl in enumerate(
        obj.get("http://www.w3.org/2004/02/skos/core#prefLabel", [])
    ):
        obj["http://www.w3.org/2004/02/skos/core#prefLabel"][i][
            ...
            [
                label,
                obj["http://www.w3.org/2004/02/skos/core#prefLabel"][i].get(
                    "@value", ""
                ),
            ]
        )
...
def _formatSubjectDocument(obj, child):
    if isinstance(obj, str):
        return (
            "?subjectId=".join([obj, str(child["_id"])])
            if (
                obj.startswith("reprolib:")
                and not (
                    obj.startswith("reprolib:terms")
                    or obj.split(":")[-1].isupper()
                )
            )
            else obj
        )
        ...
        for k in obj.keys():
            nk = (
                "?subjectId=".join([k, str(child["_id"])])
                if (
                    k.startswith("reprolib:")
                    and not (
                        k.startswith("reprolib:terms")
                        or k.split(":")[-1].isupper()
                    )
                )
                else k
            )
            n[nk] = (
                (
                    "?subjectId=".join([obj[k], str(child["_id"])])
                    if isinstance(obj[k], str)
                    else [
                        "?subjectId=".join([o, str(child["_id"])])
                        for o in obj[k]
                    ]
                ...
                if k in ["@index", "order"]
                    ...
                    "?subjectId=".join([adnK, str(child["_id"])]): ": ".join(
                        [child["displayName"], obj[k][adnK]]
                    )
                    for adnK in obj[k]
                }
                if k == "activity_display_name"
                else {
                    "?subjectId=".join([vizK, str(child["_id"])]): obj[k][vizK]
                    for vizK in obj[k]
                }
                if k == "visibility"
                else obj[k]
                if k == "@type"
                else _formatSubjectDocument(obj[k], child)
            )
        return n
    else:
        return obj
...
def reprolibPrefix(s):
    ...
    if isinstance(s, str):
        ...
        for k in s.keys():
            s[k] = (
                reprolibPrefix(s[k])
                if k not in KEYS_TO_DEREFERENCE
                else dereference(s[k])
            )
...
def reprolibCanonize(s):
    ...
    if isinstance(s, str):
        ...
        return {
            reprolibCanonize(k)
            if reprolibCanonize(k) is not None
            else k: reprolibCanonize(v)
            for k, v in s.items()
        }
    return s
...
def expandOneLevel(obj):
                    ...
                    for k in obj.keys():
                        if invalidContext in obj[k].get("@context", []):
                            ...
                            if obj[k]["@context"] is None:
                                obj[k]["@context"] = []
                            obj[k]["@context"].append(
                                reprolibCanonize(invalidContext)
                            )
    ...
    newObj = (
        newObj[0]
        if (isinstance(newObj, list) and len(newObj) == 1)
        else newObj
    )
    if isinstance(newObj, dict):
        ...
        for k, v in deepcopy(newObj).items():
            if not bool(v):
                ...
                newObj[prefix_key] = (
                    reprolibPrefix(v)
                    if prefix_key not in KEYS_TO_DEREFERENCE
                    else dereference(v)
                )
        for k in KEYS_TO_DELANGUAGETAG:
            if k in newObj.keys() and isinstance(newObj[k], list):
                newObj[k] = delanguageTag(newObj[k])
        newObj.update(
            {
                k: reprolibPrefix(obj.get(k))
                for k in obj.keys()
                if (
                    bool(obj.get(k))
                    and k not in keyExpansion(list(newObj.keys()))
                )
            }
        )
        newObj.update(
            {
                k: dereference(newObj[k])
...
def dereference(prefixed):
    ...
    if isinstance(prefixed, str):
        ...
        return {k: dereference(v) for k, v in prefixed.items()}
            ...
            if k in newObj.keys():
                ...
                if bool(v):
                    newObj[k] = (
                        ...
                        if k in KEYS_TO_DEREFERENCE
                        else reprolibPrefix(v)
                    )
        if k in KEYS_TO_DELANGUAGETAG:
            if k in newObj:
                newObj[k] = reprolibCanonize(delanguageTag(newObj[k]))
        if k in KEYS_TO_DEREFERENCE:
            if k in newObj:
...
def createCache(data, formatted, modelType, user=None, modelClasses={}):
    obj = getModel(modelClasses, modelType).findOne({"_id": data["_id"]})
    ...
    if not obj:
        print("data is", data)
        print(
            "model type is", modelType, getModel(modelClasses, modelType).name
        )
    ...
    if obj.get("cached"):
        cache_id = obj["cached"]
        saved = CacheModel().updateCache(
            cache_id,
            MODELS()[modelType]().name,
            obj["_id"],
            modelType,
            formatted,
        )
        ...
        saved = CacheModel().insertCache(
            MODELS()[modelType]().name, obj["_id"], modelType, formatted
        )
...
def clearCache(obj, modelType):
    ...
    if obj.get("cached"):
        ...
        obj["cached"] = None
        MODELS()[modelType]().update(
            {"_id": ObjectId(obj["_id"])}, {"$set": {"cached": None}}, False
        )
...
def _fixUpFormat(obj):
    if isinstance(obj, dict):
        ...
        for k in obj.keys():
            rk = reprolibPrefix(k)
            if k in KEYS_TO_DELANGUAGETAG:
                newObj[rk] = reprolibCanonize(delanguageTag(obj[k]))
    ...
    try:
        ...
        if isinstance(obj, dict):
            ...
            if all([not refreshCache, oc is not None]):
                if (
                    mesoPrefix == "item"
                    or mesoPrefix == "screen"
                    or obj.get("meta", {}).get("schema", "")
                    == APPLET_SCHEMA_VERSION
                ):
                    return loadCache(oc)
                else:
        ...
        if type(obj) == list:
            return _fixUpFormat(
                [
                    formatLdObject(
                        o, mesoPrefix, refreshCache=refreshCache, user=user
                    )
                    for o in obj
                    if o is not None
                ]
        ...
        if mesoPrefix == "applet":
            ...
            if (
                protocolUrl is not None
                and reimportFromUrl
                and protocolId
                and not protocol
            ):  # handle old schema
                protocolObj = ProtocolModel().load(
                    ObjectId(protocolId), user, force=True
                )

                if "appletId" not in protocolObj.get("meta", {}):
                    protocolObj["meta"]["appletId"] = "None"
                    ProtocolModel().setMetadata(
                        protocolObj, protocolObj["meta"]
                    )
            ...
            if protocolUrl and reimportFromUrl:
                ...
                protocol = formatLdObject(
                    protocolObj, "protocol", user, refreshCache=refreshCache
                )
            ...
            applet["protocol"] = {
                key: protocol.get(
                    "protocol", protocol.get("activitySet", {})
                ).pop(key)
                for key in [
                    "@type",
                    "_id",
            ...
            applet["applet"] = {
                ...
                **obj.get("meta", {}).get(mesoPrefix, {}),
                ...
                "retentionSettings": obj.get("meta", {}).get(
                    "retentionSettings", {}
                ),
            ...
            if len(
                obj.get("meta", {}).get("applet", {}).get("displayName", "")
            ):
                inserted = False
                ...
                for candidate in candidates:
                    for key in applet["applet"]:
                        if (
                            not inserted
                            and str(key).endswith(candidate)
                            and len(applet["applet"][key])
                            and len(applet["applet"][key][0].get("@value", ""))
                        ):
                            applet["applet"][key][0]["@value"] = obj["meta"][
                                "applet"
                            ]["displayName"]
            ...
            if obj.get("loadedFromSingleFile", False):
                activities = ActivityModel().find(
                    {
                        "meta.protocolId": obj["_id"],
                        "meta.activity": {"$exists": True},
                    }
                )
                ...
                for activity in activities:
                    formatted = formatLdObject(
                        activity, "activity", user, refreshCache=refreshCache
                    )
                    if refreshCache:
                        createCache(
                            activity, formatted, "activity", user, modelClasses
                        )

                    protocol["activities"][str(activity["_id"])] = activity[
                        "_id"
                    ]
                    ...
                    activityIDMapping[
                        "{}/{}".format(
                            str(obj["_id"]),
                            formatted.get("activity", formatted).get("@id"),
                        )
                    ] = str(activity["_id"])
                ...
                for activityFlow in activityFlows:
                    ...
                    protocol["activityFlows"][
                        str(activityFlow["_id"])
                    ] = formatted

                    activityFlowIdMapping[
                        "{}/{}".format(str(obj["_id"]), formatted.get("@id"))
                    ] = str(activityFlow["_id"])
            ...
            if obj.get("loadedFromSingleFile", False):
                ...
                for item in items:
                    ...
                    activity["items"][identifier] = formatLdObject(
                        item, "screen", user
                    )

                    key = "{}/{}".format(
                        str(item["meta"]["activityId"]), str(item["_id"])
                    )

                    itemIDMapping[
                        "{}/{}".format(
                            str(item["meta"]["activityId"]),
                            activity["items"][identifier]["@id"],
                        )
                    ] = key
                    if item.get("duplicateOf", None):
                        itemIDMapping[
                            "{}/{}".format(
                                str(item["meta"]["activityId"]),
                                str(item["duplicateOf"]),
                            )
                        ] = key

                if refreshCache and fixUpList(
                    obj, "activity", itemIDMapping, "reprolib:terms/order"
                ):
                    ActivityModel().setMetadata(obj, obj["meta"])

                activity["activity"] = _fixUpFormat(
                    obj["meta"].get("activity")
                )

            else:
                activity = {"activity": _fixUpFormat(newObj), "items": {}}
                ...
                activity = componentImport(
                    ...
                    refreshCache=refreshCache,
                    meta={
                        "protocolId": obj["meta"]["protocolId"],
                        "activityId": obj["_id"],
                    },
                )
...
def componentImport(
    ...
    from apps.girderformindlogger.models import pluralize, smartImport
    from apps.girderformindlogger.utility import firstLower
            ...
            for activity in order.get("@list", []):
                ...
                if reprolibPrefix(IRI) not in list(
                    ...
                    result = (
                        ...
                        if (
                            IRI is not None
                            and not IRI.startswith("Document not found")
                        )
                        else (None, None, None)
                    )
                    activityComponent, activityContent, canonicalIRI = result

                    activity["url"] = activity["schema:url"] = (
                        canonicalIRI if (canonicalIRI is not None) else IRI
                    )

                    activityComponent = (
                        pluralize(
                            firstLower(
                                activityContent.get("@type", [""])[0]
                                .split("/")[-1]
                                .split(":")[-1]
                            )
                        )
                        if (
                            activityComponent is None
                            and isinstance(activityContent, dict)
                        )
                    ...
                    if activityComponent is not None:
                        ...
                        if activityComponent != "screen":
                            updatedProtocol[activityComponents][
                                canonicalIRI
                            ] = content.get(activityComponent, content)[
                                "_id"
                            ].split(
                                "/"
                            )[
...
def getByLanguage(object, tag=None):
    ...
    """
    if not tag:
        from apps.girderformindlogger.api.v1.context import Context
        ...
        tag = FolderModel().findOne(
            {
                ...
                .findOne({"name": "Context"})
                .get("_id"),
            }
        )
        tag = (
            tag.get("meta", {}).get("@context", {}).get("@language")
            if tag
            else None
        )
    if isinstance(tag, str):
        ...
        if isinstance(object, dict):
            return getFromLongestMatchingKey(
                object, tags, caseInsensitive=True
            )
...
def getFromLongestMatchingKey(object, listOfKeys, caseInsensitive=True):
    ...
    key = (
        max([str(k) for k in listOfKeys], key=len) if len(listOfKeys) else None
    )
    if key and key in listOfKeys:
        listOfKeys.remove(key)
    return (
        object.get(key, getFromLongestMatchingKey(object, listOfKeys))
        if key
        else None
    )
...
def getFromLongestMatchingValue(
    objectList, listOfValues, keyToMatch, caseInsensitive=True
):
    """
    ...
    value = (
        max([str(k) for k in listOfValues], key=len)
        if len(listOfValues)
        else None
    )
    ...
    for object in sorted(
        objectList, key=lambda i: len(i.get(keyToMatch, "")), reverse=True
    ):
        if (
            object.get(keyToMatch, "").lower()
            if caseInsensitive
            else object.get(keyToMatch, "")
        ) == value:
            return object
    if len(listOfValues) >= 1:
        return getFromLongestMatchingValue(
            objectList, listOfValues, keyToMatch, caseInsensitive
        )
    for object in sorted(
        objectList, key=lambda i: len(i.get(keyToMatch, "")), reverse=False
    ):
        generic = (
            object.get(keyToMatch, "").lower()
            if caseInsensitive
...
def snake_case(camelCase):
    ...
    return all_cap_re.sub(
        r"\1_\2", first_cap_re.sub(r"\1_\2", camelCase)
    ).lower()
