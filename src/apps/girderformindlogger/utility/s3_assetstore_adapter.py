...
from apps.girderformindlogger.api.rest import setContentDisposition
...
class S3AssetstoreAdapter(AbstractAssetstoreAdapter):
    ...
    def _s3Client(connectParams):
        try:
            ...
            if "googleapis" in urllib.parse.urlparse(
                connectParams.get("endpoint_url", "")
            ).netloc.split("."):
    ...
    def validateInfo(doc):
        ...
        if doc["service"] != "":
            if not re.match(
                "^((https?)://)?([^:/]+)(:([0-9]+))?$", doc["service"]
            ):
                raise ValidationException(
                    "The service must of the form [http[s]://](host domain)[:(port)].",
                    "service",
                )
    ...
    def _getRequestHeaders(self, upload):
        headers = {
            "Content-Disposition": setContentDisposition(
                upload["name"], setHeader=False
            ),
    ...
    def _generatePresignedUrl(self, *args, **kwargs):
        ...
        if getattr(self.client, "_useGoogleAccessId", False):
            ...
            if awskey in urllib.parse.parse_qs(parsed.query):
                ...
                qsl = [
                    (key if key != awskey else gskey, value)
    ...
    def initUpload(self, upload):
        ...
        key = "/".join(
            filter(
                None,
                (self.assetstore.get("prefix", ""), uid[:2], uid[2:4], uid),
            )
        )
        ...
        requestInfo = {"headers": headers, "method": "PUT"}
        upload["behavior"] = "s3"
        upload["s3"] = {
            "chunked": chunked,
        ...
        requestInfo["url"] = self._generatePresignedUrl(
            ClientMethod=method, Params=params
        )
        return upload
    ...
    def _clientUploadChunk(self, upload, chunk):
        ...
        if "contentLength" in info and int(info["contentLength"]) != length:
            raise ValidationException(
                "Expected chunk size %d, but got %d."
    ...
    def _proxiedUploadChunk(self, upload, chunk):
        ...
        if upload["s3"]["chunked"]:
            ...
            resp = requests.request(
                method="PUT", url=url, data=chunk, headers=headers
            )
            ...
            resp = requests.request(
                ...
                data=chunk,
                headers=dict(
                    reqInfo["headers"], **{"Content-Length": str(size)}
                ),
            )
            if resp.status_code not in (200, 201):
                logger.error(
                    "S3 upload failure %d (uploadId=%s):\n%s"
                    % (resp.status_code, upload["_id"], resp.text)
                )
                raise GirderException("Upload failed (bad gateway)")
    ...
    def requestOffset(self, upload):
        ...
        return {"method": "PUT", "url": url, "headers": headers, "offset": 0}
    ...
    def downloadFile(
        ...
        **kwargs,
        ...
        params = {"Bucket": self.assetstore["bucket"], "Key": file["s3Key"]}
    ...
    def importData(self, parent, parentType, params, progress, user, **kwargs):
        ...
        pageIterator = paginator.paginate(
            Bucket=bucket, Prefix=importPath, Delimiter="/"
        )
    ...
    def fileUpdated(self, file):
        ...
        if (
            key.get("ContentType") != mime
            or key.get("ContentDisposition") != disp
        ):
            self.client.copy_object(
                Bucket=bucket,
                Key=file["s3Key"],
                Metadata=key["Metadata"],
                CopySource={"Bucket": bucket, "Key": file["s3Key"]},
                ContentDisposition=disp,
                ContentType=mime,
    ...
    def untrackedUploads(self, knownUploads=None, delete=False):
        ...
        while True:
            ...
            getParams["UploadIdMarker"] = multipartUploads[
                "NextUploadIdMarker"
            ]
    ...
    def _uploadIsKnown(self, multipartUpload, knownUploads):
        ...
        for upload in knownUploads:
            if (
                "s3" in upload
                and "uploadId" in upload["s3"]
                and "key" in upload["s3"]
            ):
                if (
                    multipartUpload["UploadId"] == upload["s3"]["uploadId"]
...
def makeBotoConnectParams(
    accessKeyId, secret, service=None, region=None, inferCredentials=False
):
    """
    ...
    if service:
        if not service.startswith("http://") and not service.startswith(
            "https://"
        ):
...
def _deleteFileImpl(event):
    event.info["client"].delete_object(
