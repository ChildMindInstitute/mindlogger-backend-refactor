...
class AccessControlMixin(object):
    ...
    def load(
        ...
        :py:func:`girderformindlogger.models.model_base.AccessControlledModel.load`.
        """
        loadFields = fields
        if not force:
        ...
        if not force and doc is not None:
            ...
            if isinstance(loadType, six.string_types):
                ModelImporter.model(loadType).load(
                    loadId, level=level, user=user, exc=exc
                )
    ...
    def hasAccess(self, resource, user=None, level=AccessType.READ):
        ...
        Takes the same parameters as
        :py:func:`girderformindlogger.models.model_base.AccessControlledModel.hasAccess`.
        """
    ...
    def hasAccessFlags(self, doc, user=None, flags=None):
        ...
        resource = ModelImporter.model(self.resourceColl).load(
            doc[self.resourceParent], force=True
        )
        return ModelImporter.model(self.resourceColl).hasAccessFlags(
            resource, user, flags
        )
    ...
    def requireAccessFlags(self, doc, user=None, flags=None):
        ...
        return ModelImporter.model(self.resourceColl).requireAccessFlags(
            resource, user, flags
        )
    ...
    def filterResultsByPermission(
        ...
        Takes the same parameters as
        :py:func:`girderformindlogger.models.model_base.AccessControlledModel.filterResultsByPermission`.
        """
        ...
        def hasAccess(_result):
            ...
            if flags:
                val = val and ModelImporter.model(
                    self.resourceColl
                ).hasAccessFlags(resource, user=user, flags=flags)
    ...
    def textSearch(
        ...
        cursor = self.findWithPermissions(
            ...
            user=user,
            level=level,
            aggregateSort=defaultSort,
        )
        if (
            sort is None
            and not getattr(cursor, "fromAggregate", False)
    ...
    def _findWithPermissionsFallback(
        self,
        query,
        offset,
        limit,
        ...
        **kwargs,
        ...
        if (
            fields
            and any(fields[key] is True for key in fields)
    ...
    def findWithPermissions(
        ...
        **kwargs,
        ...
        if level is not None and (not user or not user["admin"]):
            ...
            if (
                not isinstance(
                    ModelImporter.model(self.resourceColl),
                    AccessControlledModel,
                )
            ...
            if sort is not None or aggregateSort is not None:
                fullPipeline.append(
                    {"$sort": collections.OrderedDict(sort or aggregateSort)}
                )
            ...
            if fields is not None:
                ...
                if any(
                    not isinstance(v, abc.Mapping) for v in fields.values()
                ):
                    fullPipeline.append(
                        {
                            "$project": {
            ...
            def count():
                try:
                    return next(
                        iter(
                            self.collection.aggregate(countPipeline, **options)
                        )
                    )["count"]
