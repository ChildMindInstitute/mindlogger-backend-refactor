...
_HASHLIB_INLINE_EVP_STRUCT = _ver < (2, 7, 13) or (
    _ver >= (3,) and _ver < (3, 5, 3)
)
...
def _getHashStateDataPointer(hashObject):
    ...
    if _HASHLIB_INLINE_EVP_STRUCT:
        ...
        stateDataPointer = ctypes.cast(
            hashPointer[6], ctypes.POINTER(ctypes.c_char)
        )
    else:
        ...
        evpStruct = ctypes.cast(
            hashPointer[3], ctypes.POINTER(ctypes.c_void_p)
        )
        stateDataPointer = ctypes.cast(
            evpStruct[3], ctypes.POINTER(ctypes.c_char)
        )
