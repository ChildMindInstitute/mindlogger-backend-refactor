...
def configureServer(mode=None, plugins=None, curConfig=None):
    ...
    from apps.girderformindlogger.api import api_main
...
def loadRouteTable(reconcileRoutes=False):
    ...
    return {
        name: route for (name, route) in six.viewitems(routeTable) if route
    }
    ...
    cherrypy.tree.mount(
        ...
        "/static",
        {
            "/": {
                ...
                "request.show_tracebacks": appconf["/"][
...
class _StaticFileRoute(object):
    ...
    def GET(self):
        return cherrypy.lib.static.serve_file(
            self.path, content_type=self.contentType
        )
