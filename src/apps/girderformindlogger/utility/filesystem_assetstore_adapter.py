...
from apps.girderformindlogger.utility import mkdir, progress
...
class FilesystemAssetstoreAdapter(AbstractAssetstoreAdapter):
    ...
    def validateInfo(doc):
        ...
        if not os.path.isabs(doc["root"]):
            raise ValidationException(
                "You must provide an absolute path " "for the root directory.",
                "root",
            )
    ...
    def __init__(self, assetstore):
        ...
        try:
            ...
            logger.exception(
                "Failed to create filesystem assetstore "
                "directories %s" % self.tempDir
            )
        if not os.access(self.assetstore["root"], os.W_OK):
            self.unavailable = True
            logger.error(
                "Could not write to assetstore root: %s",
    ...
    def importFile(self, item, path, user, name=None, mimeType=None, **kwargs):
        ...
        """
        logger.debug(
            "Importing file %s to item %s on filesystem assetstore %s",
            path,
            item["_id"],
            self.assetstore["_id"],
        )
        ...
        file = File().save(file)
        logger.debug(
            "Imported file %s to item %s on filesystem assetstore %s",
            path,
            item["_id"],
            self.assetstore["_id"],
        )
    ...
    def _importDataAsItem(
        self, name, user, folder, path, files, reuseExisting=True, params=None
    ):
        params = params or {}
        ...
        events.trigger(
            ...
            {"id": item["_id"], "type": "item", "importPath": path},
        )
        for fname in files:
    ...
    def importData(
        self, parent, parentType, params, progress, user, leafFoldersAsItems
    ):
        ...
        if not os.path.isdir(importPath):
            ...
            self._importFileToFolder(
                name, user, parent, parentType, importPath
            )
            return
        ...
        if leafFoldersAsItems and self._hasOnlyFiles(importPath, listDir):
            self._importDataAsItem(
                os.path.basename(importPath.rstrip(os.sep)),
                user,
                parent,
                importPath,
                listDir,
        ...
        for name in listDir:
            ...
            if os.path.isdir(path):
                ...
                if leafFoldersAsItems and self._hasOnlyFiles(
                    ...
                    self._importDataAsItem(
                        name, user, parent, path, localListDir, params=params
                    )
                else:
                ...
                if self.shouldImportFile(path, params):
                    self._importFileToFolder(
                        name, user, parent, parentType, path
                    )

    def findInvalidFiles(
        self,
