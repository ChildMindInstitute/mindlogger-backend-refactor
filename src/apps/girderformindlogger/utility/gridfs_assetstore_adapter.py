...
def _ensureChunkIndices(collection):
    ...
    collection.create_index(
        [("uuid", pymongo.ASCENDING), ("n", pymongo.ASCENDING)], unique=True
    )
...
class GridFsAssetstoreAdapter(AbstractAssetstoreAdapter):
    ...
    def validateInfo(doc):
        ...
        if "." in doc["db"] or " " in doc["db"]:
            raise ValidationException(
                "Database name cannot contain spaces" " or periods.", "db"
            )
    ...
    def __init__(self, assetstore):
        ...
        try:
            ...
            key = (
                self.assetstore.get("mongohost"),
                self.assetstore.get("replicaset"),
            )
            if key in _recentConnections:
                recent = (
                    time.time() - _recentConnections[key]["created"]
        ...
        try:
            ...
            logger.error(
                "Failed to connect to GridFS assetstore %s",
                self.assetstore["db"],
            )
            ...
            logger.exception(
                "Failed to configure GridFS assetstore %s",
                self.assetstore["db"],
            )
    ...
    def uploadChunk(self, upload, chunk):
        ...
        try:
            ...
            self.chunkColl.delete_many(
                {"uuid": upload["chunkUuid"], "n": {"$gte": startingN}}
            )
            raise
    ...
    def downloadFile(
        ...
        **kwargs,
        ...
        cursor = self.chunkColl.find(
            {"uuid": file["chunkUuid"], "n": {"$gte": n}}, projection=["data"]
        ).sort("n", pymongo.ASCENDING)
    ...
    def deleteFile(self, file):
        ...
        if matching.count(True) == 1:
            ...
            try:
                ...
                ).delete_many({"uuid": file["chunkUuid"]})
