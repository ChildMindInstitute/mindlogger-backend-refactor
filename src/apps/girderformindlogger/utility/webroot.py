...
from apps.girderformindlogger import constants
...
class Webroot(WebrootBase):
    ...
    def _renderHTML(self):
        from apps.girderformindlogger.plugin import loadedPlugins
        from apps.girderformindlogger.utility import server
        ...
        for plugin in self.vars["plugins"]:
            if os.path.exists(
                os.path.join(builtDir, plugin, "plugin.min.css")
            ):
