...
class NoAssetstoreAdapter(GirderException):
    ...
    identifier = "girderformindlogger.utility.assetstore.no-adapter"
...
class FilePathException(GirderException):
    ...
    identifier = (
        "girderformindlogger.utility.assetstore.file-path-not-available"
    )

    def __init__(self, message="No assetstore adapter", identifier=None):
        super(FilePathException, self).__init__(
            message, identifier or self.identifier
        )
