...
async def applet_users_list(
        id_: uuid.UUID,
        user: User = Depends(get_current_user),
        query_params: QueryParams = Depends(parse_query_params(AppletUsersQueryParams)),
    ...
    return ResponseMulti(count=count, result=[PublicAppletUser.from_orm(user) for user in users])
