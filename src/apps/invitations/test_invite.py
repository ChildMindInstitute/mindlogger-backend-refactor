...
class TestInvite(BaseTest):
    ...
    async def test_admin_invite_reviewer_success(
        self, client, invitation_revier_data, tom, user, applet_one, mailbox: TestMail
    ):
        client.login(tom)
        response = await client.post(
        ...
        assert response.json()["result"]["userId"] == str(user.id)
        assert len(mailbox.mails) == 1
        assert mailbox.mails[0].recipients == [invitation_revier_data.email]
        assert mailbox.mails[0].subject == "Applet 1 invitation"
    ...
    async def test_manager_invite_reviewer_success(
        self, client, invitation_revier_data, applet_one_lucy_manager, lucy, mailbox: TestMail
    ):
        client.login(lucy)
        response = await client.post(
    ...
    async def test_coordinator_invite_reviewer_success(
        self, client, invitation_revier_data, applet_one_lucy_coordinator, lucy, mailbox: TestMail
    ):
        client.login(lucy)
        response = await client.post(
    ...
    async def test_invite_not_registered_user_reviewer(
        self, client, invitation_revier_data, tom, applet_one, mailbox: TestMail
    ):
        client.login(tom)
