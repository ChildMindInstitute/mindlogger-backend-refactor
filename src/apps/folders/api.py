...
async def folder_update_name(
    id_: uuid.UUID, data: FolderUpdate, user=Depends(get_current_user)
) -> Response[FolderPublic]:
    folder = await FolderService(user.id).update(id_, data)
    return Response(result=FolderPublic.from_orm(folder))


async def folder_delete(id_: uuid.UUID, user=Depends(get_current_user)):
    await FolderService(user.id).delete_by_id(id_)


async def folder_pin(
    id_: uuid.UUID, applet_id: uuid.UUID, user=Depends(get_current_user)
):
