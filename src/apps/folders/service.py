...
class FolderService:
    def __init__(self, session, workspace_id: uuid.UUID, user_id: uuid.UUID):
        self._workspace_id = workspace_id
        self._creator_id = user_id
        self.session = session

    async def list(self) -> list[Folder]:
        schemas = await FolderCRUD(self.session).get_users_folder_in_workspace(
            self._workspace_id, self._creator_id
        )
    ...
    async def _validate_create(self, name: str):
        existed_folder = await FolderCRUD(
            self.session
        ).get_id_of_creators_folder_by_name(
            self._workspace_id, self._creator_id, name
        )
        if existed_folder:
            raise FolderAlreadyExist()

    async def update(self, folder_id: uuid.UUID, data: FolderUpdate) -> Folder:
    ...
    async def _validate_update(self, folder_id, new_name: str):
        ...
        existed_folder_id = await FolderCRUD(
            self.session
        ).get_id_of_creators_folder_by_name(
    ...
    async def delete_by_id(self, folder_id: uuid.UUID):
    ...
    async def _validate_pin(self, folder_id: uuid.UUID, applet_id: uuid.UUID):
        ...
        applets_folders_ids: list[
            uuid.UUID
        ] = await FolderCRUD(  # type: ignore
            self.session
        ).get_applets_folder_id_in_workspace(
            self._workspace_id, applet_id
        )
        if not folder_id not in applets_folders_ids:
