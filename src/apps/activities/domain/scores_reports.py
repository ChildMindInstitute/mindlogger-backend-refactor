...
class ReportMobile(PublicModel):
    id: str
    calculation_type: CalculationType
    items_score: list[str] | None = Field(default_factory=list)
    name: str
    type: str = Field(ReportType.score, const=True)
    conditional_logic: list[ScoreConditionalLogicMobile] | None = None
...
class ScoresAndReportsMobile(PublicModel):
    show_score_summary: bool = False
    reports: list[ReportMobile] | None = Field(default_factory=list)
