...
async def main(session: AsyncSession, *args, **kwargs):
    ...
    query = query.join(ActivitySchema, ActivityItemSchema.activity_id == ActivitySchema.id)
    query = query.where(ActivitySchema.applet_id == uuid.UUID("62d06045-acd3-5a10-54f1-06f600000000"))
    query = query.where(ActivityItemSchema.response_type == ResponseType.SLIDER)
