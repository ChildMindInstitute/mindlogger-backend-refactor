...
from apps.activity_flows.domain.flow_full import FlowFull, FlowHistoryFull, FlowItemHistoryFull
from apps.activity_flows.domain.flow_history import ActivityFlowHistoryChange
from apps.activity_flows.service.flow_change import ActivityFlowChangeService, ActivityFlowItemChangeService
from apps.activity_flows.service.flow_item_history import FlowItemHistoryService
...
class FlowHistoryService:
    ...
    async def add(self, flows: list[FlowFull]):
        ...
        await FlowItemHistoryService(self.session, self.applet_id, self.version).add(flow_items)

    async def get_full(self) -> list[FlowHistoryFull]:
        schemas = await FlowsHistoryCRUD(self.session).get_by_applet_id(self.applet_id_version)
        flows = list()
        ...
        items = await FlowItemHistoryService(self.session, self.applet_id, self.version).get_by_flow_ids(flow_ids)
    ...
    async def get_changes(self, prev_version: str) -> list[ActivityFlowHistoryChange]:
        old_id_version = f"{self.applet_id}_{prev_version}"
        return await self._get_changes(old_id_version)

    async def _get_changes(self, old_id_version: str) -> list[ActivityFlowHistoryChange]:
            ...
            if not prev and new:
                flow_changes.append(
                    ActivityFlowHistoryChange(
                        name=changes_generator.added_text(f"Activity Flow by name {new.name}"),
                ...
                flow_changes.append(
                    ActivityFlowHistoryChange(name=changes_generator.removed_text(f"Activity Flow by name {prev.name}"))
                )
            elif new and prev:
                ...
                changes_items = flow_item_change_service.generate_flow_items_update(  # noqa: E501
                    self._group_and_sort_flows_or_items(getattr(new, "items", []) + getattr(prev, "items", [])),  # type: ignore
                )

                if changes or changes_items:
                    flow_changes.append(
                        ActivityFlowHistoryChange(
                            name=changes_generator.updated_text(f"Activity Flow {new.name}"),
                            changes=changes,
                            items=changes_items,
                        )
                    )
