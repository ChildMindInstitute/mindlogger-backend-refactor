...
class FlowItemService:
    ...
    async def create(self, items: list[PreparedFlowItemCreate]) -> list[ActivityFlowItemFull]:
        schemas = list()
        ...
        return [ActivityFlowItemFull.model_validate(schema) for schema in item_schemas]

    async def create_update(self, items: list[PreparedFlowItemUpdate]) -> list[ActivityFlowItemFull]:
        schemas = list()
        flow_id_ordering_map: dict[uuid.UUID, int] = defaultdict(int)
    ...
    async def get_by_flow_ids(self, flow_ids: list[uuid.UUID]) -> list[ActivityFlowItemFull]:
        schemas = await FlowItemsCRUD(self.session).get_by_flow_ids(flow_ids)
        return [ActivityFlowItemFull.model_validate(schema) for schema in schemas]
